...
struct mips_set_options
{
  ...
  /* True if MXU3 EXT ABI is used. */
  bfd_boolean mxu3_ext_abi;
};
...
struct option md_longopts[] =
{
  ...
  {"mmxu3-ext-abi", no_argument, NULL, OPTION_MXU3_EXT_ABI},
  {"mno-mxu3-ext-abi", no_argument, NULL, OPTION_NO_MXU3_EXT_ABI},
...
#define RTYPE_MXU_CR	0x2000000
#define RTYPE_MXU128	0x4000000
#define RTYPE_MXU512	0x10000000
#define RTYPE_VSR	0x20000000
#define RTYPE_VWR	0x40000000
#define RTYPE_MCS	0x80000000
...
/* OP_INT_OPT ATP EPT  matcher, for JZ mxu */
struct args {
  const char *opt;
  unsigned int value;
  enum mips_operand_type optype;
};

const struct args args_mxu_opts[] = {
  {"WW"  , 0, OP_INT_OPT},
  {"LW"  , 1, OP_INT_OPT},
  {"HW"  , 2, OP_INT_OPT},
  {"XW"  , 3, OP_INT_OPT},
...
mips_parse_argument_token (char *s, char float_format)
{
  ...
  if (float_format && float_format != 'm')
    {
  ...
  /* For mxu insn,such as d16mul xr1,xr2,xr3,xr4,WW,
    the 'WW' treated as constant instead of symbol */
  if (float_format == 'm')
    {
      char opts[6];
      int i,j = 0;
      if(!ISDIGIT(*s) && (*s) != '-')
	{
	  for (i= 0; s[i] != '\0' && s[i] != ',' && i < 5; i++)
	    opts[i] = s[i];
	  opts[i] = '\0';
	  for (j = 0; args_mxu_opts[j].opt; j++)
	    {
	      if (strcmp(args_mxu_opts[j].opt, opts) == 0)
		{
		  token.u.integer.value.X_op = O_constant;
		  token.u.integer.value.X_add_number = args_mxu_opts[j].value;
	  ...
	  as_fatal (_("args %s is invalid"), opts);
	}
    }
		    ...
		    struct mips_operand_array *operands)
{
  ...
  for (s = opcode->args; *s; ++s)
	...
	if (operand && operand->type != OP_VU0_MATCH_SUFFIX)
	  {
            if (operand->type == OP_INT_MULTI)
              {
                const struct mips_int_multi_operand *op;
                op = (const struct mips_int_multi_operand *) operand;
	        used_bits = mips_insert_operand (&op->oproot0, used_bits, -1);
	        used_bits = mips_insert_operand (&op->oproot1, used_bits, -1);
                if (op->nseg >= 3)
	          used_bits = mips_insert_operand (&op->oproot2, used_bits, -1);
                if (op->nseg >= 4)
	          used_bits = mips_insert_operand (&op->oproot3, used_bits, -1);
              }
            else 
              if (operand->type != OP_IMM0_INDEX_NOBIT)
...
md_begin (void)
{
  ...
  if ( (mips_opts.ase & ASE_MXU128) &&
      ((mips_opts.ase & ASE_MXU512) || (mips_opts.ase & ASE_MXU512_1)))
    as_fatal (_("unsupported combination: mxu2 mxu3/mxu3.1"));
  ...
  for (i = 0; i < NUMOPCODES;)
    {
      ...
      /* Part of insn names of MXU512 is same as MXU128.  */
      if (mips_opts.ase & ASE_MXU128)
        {
          if (!(mips_opcodes[i].ase & ASE_MXU512) && !(mips_opcodes[i].ase & ASE_MXU512_1))
            goto HASH_INSERT;
        }
      else if ((mips_opts.ase & ASE_MXU512) || (mips_opts.ase & ASE_MXU512_1))
        {
          if (!(mips_opcodes[i].ase & ASE_MXU128))
            {
              if (!((!strcmp (name, "law") || !strcmp (name, "lad"))
                    && !(mips_opcodes[i].ase & ASE_MXU512)))
                goto HASH_INSERT;
            }
        }
      else
        {
          if (!(mips_opcodes[i].ase & ASE_MXU128) &&
              !(mips_opcodes[i].ase & ASE_MXU512) &&
              !(mips_opcodes[i].ase & ASE_MXU512_1))
            goto HASH_INSERT;
        }
      ++i;
      continue;
...
#if 0
      if ((mips_opts.ase & ASE_MXU512) || (mips_opcodes[i].ase & ASE_MXU512))
        {
          if (!(mips_opts.ase & ASE_MXU512))
            {
              ++i;
              continue;
            }
  ...
  for (i = 0; i < 32; i++)
    {
      ...
      /* Ingenic MXU R2,R3 register.  */
      sprintf (regname, "$vr%d", i);
      symbol_table_insert (symbol_new (regname, reg_section,
				       RTYPE_MXU128 | RTYPE_MXU512 | i, 
                                       &zero_address_frag));
      /* Ingenic MXU R3 register VWR.  */
      if (i < 16)
        {
          sprintf (regname, "$vw%d", i);
          symbol_table_insert (symbol_new (regname, reg_section,
                                           RTYPE_VWR | i, 
                                           &zero_address_frag));
      }
      /* Ingenic MXU R3 register VSR.  */
      if (i < 4)
        {
          sprintf (regname, "$vs%d", i);
          symbol_table_insert (symbol_new (regname, reg_section,
                                           RTYPE_VSR | i,
                                           &zero_address_frag));
        }
      /* Ingenic MXU R3 control registgers.  */
      if (i == 0 || i == 31)
        {
...
mips_check_options (struct mips_set_options *opts, bfd_boolean abi_checks)
{
  ...
  if (opts->mxu3_ext_abi && !((opts->ase & ASE_MXU512) || (opts->ase & ASE_MXU512_1)))
    as_fatal (_("`-mmx3-ext-abi' used without a `-mmxu3/-mmxu3.1`"));
}
...
fpr_read_mask (const struct mips_cl_insn *ip)
{
  ...
  mask = insn_reg_mask (ip, ((1 << OP_REG_FP) | (1 << OP_REG_VEC)
			     | (1 << OP_REG_MSA) | (1 << OP_REG_MXU) 
                             | (1 << OP_REG_MXU128) | (1 << OP_REG_MXU512)),
...
/* OP_INT_MULTI match */
static bfd_boolean
match_int_multi_operand (struct mips_arg_info *arg,
			 const struct mips_operand *operand_base)
{
  const struct mips_int_multi_operand *operand;
  unsigned int uval;
  int min_val, max_val, factor;
  offsetT sval = -1;

  operand = (const struct mips_int_multi_operand *) operand_base;
  factor = 1 << operand->shift;
  min_val = mips_int_operand_min ((const struct mips_int_operand *)operand);
  max_val = mips_int_operand_max ((const struct mips_int_operand *)operand);
  ...
  if (operand->nseg != 2 && operand->nseg != 3 && operand->nseg != 4)
    return FALSE;

  insn_insert_operand (arg->insn, &operand->oproot0, uval >> operand->shift0);
  insn_insert_operand (arg->insn, &operand->oproot1, uval >> operand->shift1);

  if (operand->nseg >= 3)
    insn_insert_operand (arg->insn, &operand->oproot2, uval >> operand->shift2);

  if (operand->nseg >= 4)
    insn_insert_operand (arg->insn, &operand->oproot3, uval >> operand->shift3);
...
/* OP_INT_FIXED match */
static bfd_boolean
match_fixed_int_operand (struct mips_arg_info *arg,
			 const struct mips_operand *operand_base)
{
  const struct mips_fixedint_operand *operand;
  unsigned int uval;
  int min_val, max_val, factor;
  offsetT sval = -1;

  operand = (const struct mips_fixedint_operand *) operand_base;
  factor = 1 << operand->root.shift;
  /* Here, bias just is min_val */
  min_val = operand->min_val;
  max_val = operand->root.max_val;
  if (!match_const_int (arg, &sval))
    return FALSE;
  arg->last_op_int = sval;
  if (sval < min_val || sval > max_val || sval % factor)
  ...
  uval = (unsigned int) sval >> operand->root.shift;
  uval -= operand->root.bias;
	       ...
	       const struct mips_operand *operand)
{
  switch (operand->type)
    ...
    case OP_INT_FIXED:
      return match_fixed_int_operand (arg, operand);

    case OP_INT_MULTI:
      return match_int_multi_operand (arg, operand);
	...
	case BFD_RELOC_10_PCREL_S2:
	  {
	    ...
	    if (!mips_relax_branch)
	      {
		if ((address_expr->X_add_number + (1 << (shift + 9)))
		    & ~((1 << (shift + 10)) - 1))
		  as_bad (_("branch address range overflow (0x%lx)"),
			  (unsigned long) address_expr->X_add_number);
		ip->insn_opcode |= ((address_expr->X_add_number >> shift)
				    & 0x3ff) << 6;
		/* ?? Was it better to use operand->lsb */
	      }
	  }
	  break;
  ...
  for (args = opcode->args;; ++args)
	...
	case '=':
	  switch (args[1])
	    {
	    case 'g':
	      *offset_reloc = BFD_RELOC_10_PCREL_S2;
	      break;
	    }
	  break;
...
md_parse_option (int c, const char *arg)
{
  ...
  switch (c)
    ...
    case OPTION_MXU3_EXT_ABI:
      file_mips_opts.mxu3_ext_abi = TRUE;
      break;

    case OPTION_NO_MXU3_EXT_ABI:
      file_mips_opts.mxu3_ext_abi = FALSE;
      break;
...
parse_code_option (char * name)
{
  ...
  else if (strcmp (name, "mxu3extabi") == 0)
    mips_opts.mxu3_ext_abi = TRUE;
  ...
  if (fixp->fx_pcrel)
    {
      gas_assert (fixp->fx_r_type == BFD_RELOC_16_PCREL_S2
		  || fixp->fx_r_type == BFD_RELOC_10_PCREL_S2
...
mips_elf_final_processing (void)
{
  ...
  /* For Ingenic MXU512 target. */
  if (mips_opts.ase & ASE_MXU512)
    elf_elfheader (stdoutput)->e_flags |= EF_MIPS_ARCH_MXU3;
  if (file_mips_opts.mxu3_ext_abi)
    elf_elfheader (stdoutput)->e_flags |= EF_MIPS_ARCH_ASE_MXU3_EXT_ABI;

  /* Ingenic add fp64 flag. */
#if 0
  if (file_mips_opts.fp == 64)
...
static const struct mips_cpu_info mips_cpu_info_table[] =
{
  ...
  /* Ingenic Xburst and Xburst2 */
  { "xburst",         0, 0,	                ISA_MIPS32R2, CPU_MIPS32R2 },
  { "xburst2",        0, ASE_MSA,	        ISA_MIPS32R2, CPU_MIPS32R2 },
  /* M5100 family */
...
-mno-mxu		do not generate MXU instructions\n"));
  fprintf (stream, _("\
-mmxu2			generate MXU R2 instructions\n\
-mno-mxu2		do not generate MXU R2 instructions\n"));
  fprintf (stream, _("\
-mmxu3			generate MXU R3 instructions\n\
-mno-mxu3		do not generate MXU R3 instructions\n"));
  fprintf (stream, _("\
-mmxu3.1		generate MXU R3.1 instructions\n\
-mno-mxu3.1		do not generate MXU R3.1 instructions\n"));
  fprintf (stream, _("\
