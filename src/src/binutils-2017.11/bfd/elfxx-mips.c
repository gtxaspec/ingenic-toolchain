			       ...
			       bfd_boolean save_addend)
{
  ...
  *cross_mode_jump_p = (!bfd_link_relocatable (info)
  ...
  switch (r_type)
    ...
    case R_MIPS_PC10:
      addend >>= howto->bitpos;
      value = symbol + _bfd_mips_elf_sign_extend (addend, 12) - p;
      overflowed_p = mips_elf_overflow_p (value, 12);
...
mips_elf_merge_obj_e_flags (bfd *ibfd, bfd *obfd)
{
  ...
  if ((new_flags & EF_MIPS_ARCH_ASE) != (old_flags & EF_MIPS_ARCH_ASE))
      ...
      if (m16_mis || micro_mis)
	{
      ...
      /* Compare MXU3 ABI. */
      int old_mxu3 = old_flags & EF_MIPS_ARCH_MXU3;
      int new_mxu3 = new_flags & EF_MIPS_ARCH_MXU3;
      int old_mxu3_ext_abi = old_flags & EF_MIPS_ARCH_ASE_MXU3_EXT_ABI;
      int new_mxu3_ext_abi = new_flags & EF_MIPS_ARCH_ASE_MXU3_EXT_ABI;
      if (old_mxu3 == new_mxu3)
        {
          if (old_mxu3_ext_abi != new_mxu3_ext_abi)
            {
              _bfd_error_handler (_("%B: linking %s module with previous %s modules"),
                ibfd,
                (new_flags & EF_MIPS_ARCH_ASE_MXU3_EXT_ABI
                 ? "-mmxu3-ext-abi" : "-mno-mxu3-ext-abi"),
                (old_flags & EF_MIPS_ARCH_ASE_MXU3_EXT_ABI
                 ? "-mmxu3-ext-abi" : "-mno-mxu3-ext-abi"));
              ok = FALSE;
            }
        }
  ...
  // TODO
  if ((old_flags & EF_MIPS_ARCH_MXU3) || (new_flags & EF_MIPS_ARCH_MXU3))
    {
      elf_elfheader (obfd)->e_flags |= EF_MIPS_ARCH_MXU3;
    }
  new_flags &= ~EF_MIPS_ARCH_MXU3;
  old_flags &= ~EF_MIPS_ARCH_MXU3;
  ...
  /* Compare FP64 state.  */
#if 0
  // Just for Ingenic, nan2008 and fp64 must be combination for Xburst2
  if (new_flags & EF_MIPS_NAN2008)
    new_flags |= EF_MIPS_FP64;
  if (old_flags & EF_MIPS_NAN2008)
  ...
  else if ((new_flags & EF_MIPS_FP64) || (old_flags & EF_MIPS_FP64))
    elf_elfheader (obfd)->e_flags |= EF_MIPS_FP64;
  new_flags &= ~EF_MIPS_FP64;
  old_flags &= ~EF_MIPS_FP64;
#else
  if ((new_flags & EF_MIPS_FP64) != (old_flags & EF_MIPS_FP64))
...
_bfd_mips_elf_print_private_bfd_data (bfd *abfd, void *ptr)
{
  ...
  if (elf_elfheader (abfd)->e_flags & EF_MIPS_ARCH_MXU3)
    {
      fprintf (file, " [mxu3]");
      if (elf_elfheader (abfd)->e_flags & EF_MIPS_ARCH_ASE_MXU3_EXT_ABI)
        fprintf (file, " [mxu3-ext-abi]");
    }
