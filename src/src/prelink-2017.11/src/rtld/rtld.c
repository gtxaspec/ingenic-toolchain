...
#include <unistd.h>
#ifdef _WIN32
#define basename avoid_basename
#endif
#include <libgen.h>
#ifdef _WIN32
#undef basename
	      ...
	      if (dyn.d_tag == DT_NEEDED)
		{
		  ...
		  if (new_dso_ent == NULL)
		      ...
		      if (new_name == 0 || wrap_access (new_name, R_OK) < 0)
			{
			 ...
			 some symlink). */
		      new_canon_name = prelink_canonicalize (new_name, &st);
...
main(int argc, char **argv)
{
  ...
  /* Don't canonicalize sysroot path under Windows to avoid transforming
     Cygwin-style path into Windows-style path.  Doing otherwise will produce
     Windows-style paths in the output out of Cygwin-style path supplied as
     the input.  Canonicalization will still happen on per-file basis for
     internal bookkeeping and matching.  */
#else
  if (sysroot)
  ...
  while (remaining < argc)
    {
      ...
      if (host_paths)
#ifdef _WIN32
	fd = open (argv[remaining], O_RDONLY | O_BINARY);
#else
