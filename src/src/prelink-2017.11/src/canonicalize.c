		...
		int allow_missing)
{
  ...
  for (np = strchr (name, '\\'); np; np = strchr (np + 1, '\\'))
    *np = '/';
#endif
...
#if defined _WIN32 && ! defined __CYGWIN__
  if (chroot[0] && cygpath (chroot, win32_path))
    chroot = strdup (win32_path);
  else
    chroot = strdup (chroot);
  if (chroot == NULL)
    {
      errno = ENOMEM;
      return NULL;
    }
  for (np = strchr (chroot, '\\'); np; np = strchr (np + 1, '\\'))
    *np = '/';
#endif
...
#if defined _WIN32 && ! defined __CYGWIN__
      for (np = strchr (rpath, '\\'); np; np = strchr (np + 1, '\\'))
	*np = '/';
#endif
...
#if defined _WIN32 && ! defined __CYGWIN__
      strcpy (dest, name);
      if (cygpath ((chroot_len > 0 ? rpath : name), win32_path))
	{
	  strcpy (rpath, win32_path);
	  for (np = strchr (rpath, '\\'); np; np = strchr (np + 1, '\\'))
	    *np = '/';
	  dest = rpath + strlen (rpath);
	  if (lstat64 (rpath, stp) < 0 && ! allow_missing)
	    goto error;
	  stp_initialized = 1;
	  goto done;
	}
