...
*/
  if (j != k) {
    printf("assert(j == k) at dso.c:376 has failed for file %s\n", name);
    goto error_out;
  }
...
# ifndef __linux__
  ...
  if (plarch_p == __stop_pl_arch || ehdr.e_machine == EM_NONE)
    {
...
reopen_dso (DSO *dso, struct section_move *move, const char *temp_base)
{
  ...
  if (elf == NULL)
    {
      error (0, 0, "cannot open ELF file: %s (elf_begin failed)", elf_errmsg (-1));
      goto error_out;
