...
execstack_make_rdwr (DSO *dso, int flag)
{
  ...
  if (prelink_path == NULL)
    {
      dirname = make_relative_prefix (program_path, BINDIR, BINDIR);
  ...
  argv[2] = filename;
  argv[3] = dso->filename;
  errstring = pex_one (PEX_LAST | PEX_SEARCH, prelink_path,
                       argv, PRELINK_PROG, NULL, NULL, &statuscode, &errcode);
  if (errstring != NULL)
    {
      error (0, 0, "prelink --undo failed: %s", errstring);
      goto error_out;
    }
#endif
