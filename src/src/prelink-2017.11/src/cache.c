...
prelink_load_cache (void)
{
  ...
  mapfile_handle map_handle;
  char* buffer;

  switch (mapfile_create_mapping(prelink_cache, &buffer, &map_handle))
    {
    case MAPFILE_OPEN_FAILURE:
      return 0;

    case MAPFILE_ZERO_LENGTH:
      return 0;

    case MAPFILE_MAP_FAILED:
#ifdef _WIN32
      error (EXIT_FAILURE, GetLastError (),
             "mmap of prelink cache file failed.");
#else
    error (EXIT_FAILURE, errno, "mmap of prelink cache file failed.");
#endif

    case MAPFILE_SUCCESS:
      break;
    }

  cache = (struct prelink_cache*) buffer;
  ...
  if (memcmp (cache->magic, PRELINK_CACHE_MAGIC,
	...
	error (EXIT_FAILURE, 0, "%s: is not prelink cache file",
	       prelink_cache);
      mapfile_close_mapping(&map_handle);
      return 0;
    }
...
prelink_print_cache_object (void **p, void *info)
{
  ...
  if (e->type == ET_CACHE_DYN)
    printf (convert_format ("%s [0x%08x] 0x%0*llx-0x%0*llx%s\n"),
...
prelink_save_cache (int do_warn)
{
  ...
  if (fd < 0)
    {
      error (0, errno, "Could not write prelink cache temporary");
      return 1;
    }

#ifdef _WIN32
  wrap_unlink (prelink_cache);
#endif
...
prelink_entry_restorefn (FILE *f)
{
  ...
  p = q + 1;
  if (sscanf (p,
              convert_format ("%x|%x|%llx|%llx|%llx|%llx|%llx|%d|%d|%d|%d|%d|%d|%d|%n"),
