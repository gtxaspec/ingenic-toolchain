...
#include "missing.h"
#include "elf-macosx.h"
#else
/* Assume Solaris if none of the above.  */
#include "elf-solaris.h"
#include <alloca.h>
#include "missing.h"
#endif
...
#if defined (__linux__) || defined (_WIN32) || defined(__APPLE_CC__)
#include <stdint.h>
#else
/* Solaris. */
typedef int (*__nftw64_func_t)(const char *, const struct stat64 *,
                               int, struct FTW *);
#endif
#include <stdlib.h>
#include <stdio.h>
...
#ifdef _WIN32
char *convert_format (const char *);
#else
# define convert_format(s) s
#endif
...
#ifndef __linux__
#define PL_ARCH(F) \
struct PLArch plarch_##F
#elif (__GNUC__ > 3 || (__GNUC__ == 3 && __GNUC_MINOR__ >= 3))
#define PL_ARCH(F) \
static struct PLArch plarch_##F __attribute__((section("pl_arch"),used))
#else
#define PL_ARCH(F) \
static struct PLArch plarch_##F __attribute__((section("pl_arch"),unused))
#endif
