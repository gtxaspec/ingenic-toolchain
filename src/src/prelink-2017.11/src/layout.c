...
#ifdef _WIN32
extern struct PLArch plarch_arm;
#endif
...
layout_libs (void)
{
  ...
  for (arch = 0; arch < narches; ++arch)
...
#ifndef __linux__
      plarch = NULL;
      for (plarch_p = __start_pl_arch; plarch_p < __stop_pl_arch; plarch_p++)
        { plarch = *plarch_p;
	...
	{
	  printf (convert_format ("Laying out %d libraries in virtual address space %0*llx-%0*llx\n"),
		  l.nlibs - j, class == ELFCLASS32 ? 8 : 16, (long long) mmap_base,
		  class == ELFCLASS32 ? 8 : 16, (long long) mmap_end);
	  if (mmap_start != mmap_base)
	    printf (convert_format ("Random base 0x%0*llx\n"),
...
not_found:
      ...
      if (verbose)
	{
	  ...
	  for (i = 0; i < l.nlibs; ++i)
	    if (l.libs[i]->done >= 1)
	      printf (convert_format ("%-60s %0*llx-%0*llx\n"), l.libs[i]->filename,
