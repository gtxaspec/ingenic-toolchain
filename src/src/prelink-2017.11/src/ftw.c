...
#ifdef __CYGWIN__
/* Time saver: Don't run the configure tests since we only need them for ftw... */
...
# define __getcwd(P, N) getcwd (P, N)
#else
...
# else
#ifdef _WIN32
#  define LXSTAT(V,f,sb) stat_win32 (f,sb)
#  define XSTAT(V,f,sb) stat_win32 (f,sb)
#else
...
#ifdef _WIN32
/* Special wrapper for "stat" on Win32 to strip any trailing slash. */
int stat_win32(const char* filename, struct stat* st)
{
  ...
  if (filename[filename_length - 1] == '\\'
      || filename[filename_length - 1] == '/')
    {
      ...
      filename_copy[filename_length - 1] = '\0';
    }

  ret = stat (filename_copy ? filename_copy : filename, st);
