			   ...
			   bool include_delayed_effects)
{
  ...
  switch (code)
    ...
    /* If the CALL_INSN or the sequence which includes a CALL_INSN
       is followed with a BARRIER insn, it is a sibling call and may
       reference what the end_of_function_needs' needs. */
    {
      rtx_insn *next_insn = sequence != 0
        ? NEXT_INSN (as_a <rtx_insn *> (sequence->element (seq_size-1)))
        : NEXT_INSN (as_a <rtx_insn *> (x));
      if (next_insn != NULL && BARRIER_P (next_insn))
        IOR_HARD_REG_SET (res->regs, end_of_function_needs.regs);
    }
