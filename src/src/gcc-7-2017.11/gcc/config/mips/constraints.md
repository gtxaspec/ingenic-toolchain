...
(define_register_constraint "q" "COP2_REGS"
  "COP2 register (if available).")
...
(define_register_constraint "T" "VSR_REGS"
  "@internal")

(define_register_constraint "w" "VWR_REGS"
  "@internal")
...
(define_register_constraint "C" "ISA_HAS_MXU3 ? NO_REGS : COP2_REGS"
  "@internal")
...
(define_constraint "YN"
  "@internal
   A replicated vector const in which the replicated value is in the range
   [-16384,16383]."
  (and (match_code "const_vector")
       (match_test "mips_const_vector_same_int_p (op, mode, -16384, 16383)")))

(define_constraint "YX"
  "@internal
   A replicated vector const in which the replicated value is 16bits imm."
  (and (match_code "const_vector")
       (ior (and (match_test "mode == V64QImode")
                 (match_test "mips_const_vector_same_int_p (op, mode, -256, 255)"))
            (and (match_test "mode == V32HImode")
                 (match_test "mips_const_vector_same_int_p (op, mode, -32768, 32767)"))
            (and (match_test "mode == V16SImode")
                 (match_test "mips_const_vector_same_int_p (op, mode, -2147483648, 2147483647)"))
            (and (match_test "mode == V1SImode")
                 (match_test "mips_const_vector_same_int_p (op, mode, -512, 511)")))))
