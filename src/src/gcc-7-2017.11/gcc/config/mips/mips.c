...
#define MIPS_FTYPE_NAME5(A, B, C, D, E, F) MIPS_##A##_FTYPE_##B##_##C##_##D##_##E##_##F
...
struct mips_arg_info {
  ...
  /* True if the argument is passed in a vec register,
     valid only if the TARGET_MXU3_EXT_ABI is enabled. */
  bool vpr_p;
...
const enum reg_class mips_regno_to_class[FIRST_PSEUDO_REGISTER] = {
  ...
  VSR_REGS,	VSR_REGS,	VSR_REGS,	VSR_REGS,
  VWR_REGS,	VWR_REGS,	VWR_REGS,	VWR_REGS,
  VWR_REGS,	VWR_REGS,	VWR_REGS,	VWR_REGS,
  VWR_REGS,	VWR_REGS,	VWR_REGS,	VWR_REGS,
  VWR_REGS,	VWR_REGS,	VWR_REGS,	VWR_REGS
};
...
/* For Ingenic MXU3 */
static tree mips_mxu3_unsigned_intOI_type_node = NULL_TREE;
...
{
  /* MSA and MXU2 LD.* and ST.* cannot support loading symbols via an immediate
     operand.  */
  if (MSA_SUPPORTED_MODE_P (mode) || MXU2_SUPPORTED_MODE_P (mode)
      || MXU3_SUPPORTED_MODE_P (mode))
    return 0;
...
mips_valid_offset_p (rtx x, machine_mode mode)
{
  ...
  if (ISA_HAS_MXU3)
  {
    if (MXU3_SUPPORTED_MODE_P (mode)
        && !mips_signed_immediate_p (INTVAL(x), 5, mips_ldst_scaled_shift (mode)))
      return false;
    else
      return true;
  }
...
mips_valid_lo_sum_p (enum mips_symbol_type symbol_type, machine_mode mode)
{
  ...
  /* MSA and MXU2 LD.* and ST.* cannot support loading symbols via %lo($base).  */
  if (MSA_SUPPORTED_MODE_P (mode) || MXU2_SUPPORTED_MODE_P (mode))
    return false;
...
mips_address_insns (rtx x, machine_mode mode, bool might_split_p)
{
  ...
  bool mxu3_p = (!might_split_p && MXU3_SUPPORTED_MODE_P (mode));
	...
	if (msa_p || mxu2_p)
	  {
        ...
        if (mxu3_p)
          {
            /* The MXU3 MODE512 will be splited into 2 MODE256 insns,
             * so necessary to ensure that the offset of the high-part does not overflow. */
            HOST_WIDE_INT offset = INTVAL (addr.offset);
            if ((GET_MODE_SIZE (mode) == 64) && (offset > 0))
              offset += 32;
	    if (mips_signed_immediate_p (offset, 5, 
...
mips_ldst_scaled_shift (machine_mode mode)
{
  ...
  if (ISA_HAS_MXU3)
    {
      shift = exact_log2 (GET_MODE_SIZE (mode));
      /* The MODE512 will be splited into 2 MODE256 insns,
       * so make the scaled shift as same as MODE256. */
    ...
    shift = exact_log2 (GET_MODE_UNIT_SIZE (mode));
...
mips_const_insns (rtx x)
{
  ...
  switch (GET_CODE (x))
    ...
    case CONST_VECTOR:
	  ...
	  || ((ISA_HAS_MXU3 && MXU3_SUPPORTED_MODE_P(GET_MODE (x))
         && ((GET_MODE (x) == V16SImode &&
              mips_const_vector_same_int_p (x, GET_MODE (x), -2147483648, 2147483647))
          || (GET_MODE (x) == V32HImode &&
              mips_const_vector_same_int_p (x, GET_MODE (x), -32768, 32767))
          || (GET_MODE (x) == V64QImode &&
              mips_const_vector_same_int_p (x, GET_MODE (x), -256, 255))
          || (GET_MODE (x) == V1SImode &&
              mips_const_vector_same_int_p (x, GET_MODE (x), -512, 511))))))
	return 1;
      /* Fall through.  */
...
mips_mxu3_offset_high_part (HOST_WIDE_INT val, machine_mode mode)
{
  HOST_WIDE_INT bits = 5 + mips_ldst_scaled_shift (mode);
  return ((val + (1 << (bits - 1))) & ~(unsigned HOST_WIDE_INT)((1 << bits) - 1));
}

/* Return a memory reference like MEMREF, but with a valid
 * addressing mode. */

static rtx
mips_add_offset_512bit (rtx base, HOST_WIDE_INT offset, machine_mode mode)
{
  rtx addr, high;

  high = gen_int_mode (mips_mxu3_offset_high_part (offset, mode), Pmode);
  offset = offset - mips_mxu3_offset_high_part (offset, mode);

  high = mips_force_temporary (NULL, high);
  base = mips_force_temporary (NULL, gen_rtx_PLUS (Pmode, high, base));
			 ...
			 machine_mode mode)
{
  ...
  if (offset != 0)
	...
	base = copy_to_mode_reg (Pmode, base);
      if (MXU3_SUPPORTED_MODE_P (mode))
        addr = mips_add_offset_512bit (base, offset, mode);
      else
...
{
  /* V1SImode only for VWR REGS in MXU3. */
  if (ISA_HAS_MXU3 && mode == V1SImode)
    {
      if (GET_CODE (src) == SUBREG)
        {
          machine_mode imode = GET_MODE (SUBREG_REG (src));
          if (imode == V16SImode || imode == V16SFmode)
            {
              rtx reg = SUBREG_REG (src);
              if (imode != V16SImode)
                reg = simplify_gen_subreg (V16SImode, reg, imode, 0);

              emit_insn (gen_mxu3_movw_v1si (dest, reg,
                         GEN_INT (SUBREG_BYTE (src) >> 2)));
              return true;
            }
        }

      emit_insn (gen_mxu3_vwr_move (dest, src));
      return true;
    }

  /* Note since we force assign V1SImode pseduo regs to VWR_REGS,
     the special case such as:

     ```
        (set (mem:SI (addr))
            (subreg:SI (reg:V1SI r*) 0))
     ```
     should be manually translated to:
     ```
       (set (mem:V1SI (addr))
          (reg:V1SI r*))
     ```
     to avoid the infinity loops during reloading. */
  if (ISA_HAS_MXU3 && mode == SImode
      && memory_operand (dest, mode)
      && register_operand (src, mode)
      && GET_CODE (src) == SUBREG
      && GET_MODE (SUBREG_REG (src)) == V1SImode)
    {
      /* Make sure the offset of addr is valid for SAW. */
      rtx addr = mips_force_address (XEXP (dest, 0), V1SImode);

      dest = gen_rtx_MEM (V1SImode, addr);
      src = SUBREG_REG (src);
...
/* Return true if value X is constant with one bit is 1 and other bit is zero and is legal.  */
bool
mips_mxu3_1bit_select_p (rtx x, machine_mode mode)
{
  int val;
  ...
  switch (mode)
    ...
    case V2OImode:
      return ((val == (1 <<  0)) || (val == (1 <<  1))) ? true : false;
    default:
      return false;
    }
}
...
mips_set_reg_reg_piece_cost (machine_mode mode, unsigned int units)
{
  if ((ISA_HAS_MSA && MSA_SUPPORTED_MODE_P(mode))
      || (ISA_HAS_MXU2 && MXU2_SUPPORTED_MODE_P(mode))
      || (ISA_HAS_MXU3 && MXU3_SUPPORTED_MODE_P(mode)))
    return COSTS_N_INSNS (1);
  else
...
mips_split_move_p (rtx dest, rtx src, enum mips_split_type split_type)
{
  ...
  /* Check if MSA and MXU2 moves need splitting.  */
  if (MSA_SUPPORTED_MODE_P (GET_MODE (dest))
      || MXU2_SUPPORTED_MODE_P (GET_MODE (dest)))
    return mips_split_128bit_move_p (dest, src);

  /* Check if MXU3 moves need splitting.  */
  if (MXU3_SUPPORTED_MODE_P (GET_MODE (dest)))
    return mips_split_512bit_move_p (dest, src);
  
  /* Check if MXU3 law/saw insns need splitting. */
  if (ISA_HAS_MXU3 && GET_MODE (dest) == SImode)
    {
      rtx reg, mem;

      reg = REG_P (src) ? src : REG_P (dest) ? dest : NULL_RTX;
      mem = MEM_P (src) ? src : MEM_P (dest) ? dest : NULL_RTX;

      if (reg != NULL_RTX && VWR_REG_P (REGNO (reg)))
        return true;

      if (reg != NULL_RTX && mem != NULL_RTX 
          && MXU3_REG_P (REGNO (reg))
          && !mips_legitimate_address_p (V1SImode, XEXP (mem, 0), false))
        return true;
    }
...
mips_split_move (rtx dest, rtx src, enum mips_split_type split_type)
{
  ...
  else if (MXU3_SUPPORTED_MODE_P (GET_MODE (dest))
           && GET_MODE (dest) != V1SImode)
    mips_split_512bit_move (dest, src);
  ...
  else if (GET_MODE (dest) == SImode)
    {
      /* Legitimize the address format for LAW instructions. 
         Note we cannot do this in mips_legitimize_address because
         it has not information about the context of the address other
         than its machine_mode. Maybe the compiler assumes all machine 
         load/store insns should use the same format for the same mode.
         However, in our case, if we assign SImode into a MXU3_REG, 
         only a 10-bit uimm offset rather than 16-bit one is allowed.*/
      rtx mem, addr, base, off;

      mem = MEM_P (dest) ? dest : src;
      ...
      base = XEXP (addr, 0);
      off = XEXP (addr, 1);
      gcc_assert (REG_P (base) && CONST_INT_P (off));

      addr = gen_rtx_REG (Pmode, REGNO (base));

      mips_emit_move (addr, gen_rtx_PLUS (Pmode, base, off));
      
      if (dest == mem)
        mips_emit_move (gen_rtx_MEM (SImode, addr), src);
      else
        mips_emit_move (dest, gen_rtx_MEM (SImode, addr));

      off = gen_rtx_CONST_INT (SImode, -INTVAL (off));
      mips_emit_move (addr, gen_rtx_PLUS (Pmode, base, off));
    }
  else
    {
...
/* Return true if a 512-bit move from SRC to DEST should be split.  */

bool
mips_split_512bit_move_p (rtx dest, rtx src)
{
  if (GET_MODE (dest) != GET_MODE(src))
    return false;

  /* Check for MXU3 loads and stores.  */
  if (MXU3_REG_RTX_P (dest) && MEM_P (src) && GET_MODE_SIZE (GET_MODE(dest)) == 64)
    return true;

  if (MXU3_REG_RTX_P (src) && MEM_P (dest) && GET_MODE_SIZE (GET_MODE(src)) == 64)
    return true;
...
bool
mips_mxu3_fix_bug_for_a1_split_p (rtx_insn *insn)
{
  rtx_insn *curr = insn;
  rtx_insn *prev;
  int cnt = 0, max_cnt = 8, bug_insn = 0;

  if (!(cfun->machine->machine_reorg2_p && reload_completed))
    return false;

  for (prev = PREV_INSN (curr);
       prev && !NOTE_INSN_BASIC_BLOCK_P (prev);
       prev = PREV_INSN (prev))
    {
      if (NONDEBUG_INSN_P (prev) && !NOTE_P (prev))
        {
	  rtx x = PATTERN (prev);
	  ...
	  machine_mode mode = GET_MODE (dest);
          if (!REG_P (dest) || !MXU3_SUPPORTED_MODE_P (mode))
	    continue;
	  
	  int is_load_sync = get_attr_mxu3_type (prev) == MXU3_TYPE_LOAD_SYNC;
	  int is_load_bug = get_attr_mxu3_type (prev) == MXU3_TYPE_LOAD_BUG;
	  int is_load = is_load_sync + is_load_bug;
          gcc_assert (is_load < 2);
	  cnt += is_load;

	  if (mode == V16SImode || mode == V8DImode)
            bug_insn |= is_load;
...
mips_split_128bit_move_p (rtx dest, rtx src)
{
  ...
  /* MXU2-to-MXU2 moves can be done in a single instruction.  */
  ...
  /* Check for MXU2 loads and stores.  */
  if (MXU2_REG_RTX_P (dest) && MEM_P (src))
    return false;
  if (MXU2_REG_RTX_P (src) && MEM_P (dest))
    return false;

  /* Check for MSA and MXU2 set to an immediate const vector with valid replicated
     element.  */
  ...
  if ((FP_REG_RTX_P (dest) || MXU2_REG_RTX_P (dest))
...
mips_split_128bit_move (rtx dest, rtx src)
{
  ...
  if (FP_REG_RTX_P (dest))
	...
	{
	  s = mips_subword_at_byte (src, byte);
	  emit_insn (gen_mxu2_insfcpu_w (new_dest, new_dest, GEN_INT (index), s));
	}
    }
...
/* Same as simplify_subreg but handle the case when the op
   is volatile mem location. */

static rtx
mips_mxu3_simplify_subreg (machine_mode outermode, rtx op, 
    machine_mode innermode, unsigned int bytes)
{
  gcc_assert (outermode == OImode 
              && GET_MODE_SIZE (innermode) == UNITS_PER_MXU3_REG);

  if (! MEM_P (op) || ! MEM_VOLATILE_P (op))
    return simplify_subreg (outermode, op, innermode, bytes);

  if (! mode_dependent_address_p (XEXP (op, 0), MEM_ADDR_SPACE (op)))
    return adjust_address_nv (op, outermode, bytes);

  return NULL_RTX;
}

/* Lowering a 256-bit move to MXU3 lao/sao */

void
mips_lowering_256bit_move (rtx data, rtx addr, int p, bool is_store)
{
  rtx base, offset;
  ...
  if (GET_CODE(XEXP (addr, 0)) == REG)
    ...
    {
      gcc_assert ((GET_CODE (XEXP (XEXP (addr, 0), 0)) == REG) &&
  ...
  if (is_store)
    emit_insn (gen_mxu3_sao_insn (data, GEN_INT (p), base, offset));
  else
    emit_insn (gen_mxu3_lao_insn (data, data, GEN_INT (1 << p), base, offset));

  return;
}

/* Split a 512-bit move from SRC to DEST.  */

void
mips_split_512bit_move (rtx dest, rtx src)
{
  rtx new_src, new_dest;
  machine_mode src_mode = GET_MODE (src), dest_mode = GET_MODE (dest);
 
  if (GET_CODE (dest) == MEM)
    {
      new_dest = mips_mxu3_simplify_subreg (OImode, dest, dest_mode, 0);
      new_src = simplify_subreg (V2OImode, src,  src_mode,  0);
      mips_lowering_256bit_move (new_src, new_dest, 0, true);

      new_dest = mips_mxu3_simplify_subreg (OImode, dest, dest_mode, 32);
      new_src = simplify_subreg (V2OImode, src,  src_mode,  0);
      mips_lowering_256bit_move (new_src, new_dest, 1, true);
    }
  else
    {
      new_src = mips_mxu3_simplify_subreg (OImode, src,  src_mode,  0);
      new_dest = simplify_subreg (V2OImode, dest, dest_mode, 0);
      mips_lowering_256bit_move (new_dest, new_src, 0, false);

      new_src = mips_mxu3_simplify_subreg (OImode, src,  src_mode,  32);
      new_dest = simplify_subreg (V2OImode, dest, dest_mode, 0);
      mips_lowering_256bit_move (new_dest, new_src, 1, false);
    }
}
...
/* Split a MXU2 insfcpu.D with operand DEST, SRC1.INDEX and SRC2.  */
...
/* Split a mtcpuD  with operands  DEST, SRC and INDEX. GEN is a function
 * used to generate subregs.  */
...
mips_output_move (rtx dest, rtx src)
{
  ...
  bool mxu3_p = MXU3_SUPPORTED_MODE_P (mode);
  enum mips_symbol_type symbol_type;
  ...
  else if (mxu3_p && dest_code == REG && MXU3_REG_RTX_P (dest)
	   && src_code == CONST_VECTOR)
    {
      if (CONST_INT_P (CONST_VECTOR_ELT (src, 0)))
        {
          gcc_assert (mips_const_vector_same_val_p (src, mode));
          HOST_WIDE_INT val = INTVAL (CONST_VECTOR_ELT (src, 0));

          if (MXU3_REG_P (REGNO (dest)))
            {
              if (mode == V16SImode)
                {
                  if ((val >= -32768) && (val <= 32767))
                    ...
                    {
                      if (((unsigned int)val >> 16) != ((unsigned int)val & 0xffff))
                        return "liw\t%w0, %I1\n\tliwh\t%w0, %J1";
                      else
                        return "lih\t%w0, %H1";
                    }
                }
              else if (GET_MODE (dest) == V32HImode)
                return "lih\t%w0, %H1";
              else if (GET_MODE (dest) == V64QImode)
                return "lih\t%w0, %Q1";
              else if (!val)
                return "liw\t%w0, %E1";
              else
        ...
        if (VSR_REG_P (REGNO (dest)) && !val)
          return "sumz\t%w0";
      }

      if (mode == V16SFmode && src == CONST0_RTX (mode))
        {
          if (MXU3_REG_P (REGNO (dest)))
            return "liw\t%w0, 0";
          else if (VSR_REG_P (REGNO (dest)))
            return "sumz\t%w0";
          else
            gcc_unreachable ();
        }
  ...
  if ((src_code == REG && GP_REG_P (REGNO (src)))
      ...
      if (dest_code == REG)
	{
	  ...
	  if (MXU2_REG_P(REGNO (dest)) && mxu2_p)
	    {
	      gcc_assert (src == CONST0_RTX (GET_MODE (src)));
	      return "li%v0\t%w0,0";
	    }
  ...
  if (src_code == REG && MXU2_REG_P (REGNO (src)))
    {
      if (mxu2_p)
	{
  ...
  if (src_code == REG && MXU3_REG_P (REGNO (src)))
    {
      if (mxu3_p)
	{
	  if (dest_code == REG &&  MXU3_REG_P (REGNO (dest)))
	    return "orv\t%w0,%w1,%w1";

	  if (dest_code == REG && VSR_REG_P (REGNO (dest)))
	    return "mtsum\t%w0,%w1";

	  if (dest_code == MEM)
            gcc_unreachable ();
	}
      if (mode == SImode && dest_code == MEM)
        {
          gcc_assert (mips_legitimate_address_p (V1SImode, XEXP (dest, 0), false));
          return "saw\t%w1[0],%0";
        }
    }

  if (mxu3_p && src_code == REG &&  VSR_REG_P (REGNO (src)))
    {
      if (dest_code == REG &&  MXU3_REG_P (REGNO (dest)))
	return "mfsum\t%w0,%w1";
    }
  ...
  if (dest_code == REG && MXU2_REG_P (REGNO (dest)))
   {
      if (src_code == MEM && mxu2_p)
	return "lu1q\t%w0,%1";
   }

  if (dest_code == REG && mode == SImode)
    {
      if (MXU3_REG_P (REGNO (dest)))
        {
  if (src_code == MEM)
    {
      gcc_assert (mips_legitimate_address_p (V1SImode, XEXP (src, 0), false));
      return "law\t%w0[0],%1";
    }
  if (src_code == REG && MXU3_REG_P (REGNO (src)))
    return "movw\t%w0[0],%w1[0]";
        }
    }
...
{
  mips_expand_msa_branch(operands, gen_fn);
}

void
mips_expand_mxu3_branch (rtx *operands, rtx (*gen_fn)(rtx, rtx, rtx))
{
  mips_expand_msa_branch(operands, gen_fn);
}
...
mips_init_cumulative_args (CUMULATIVE_ARGS *cum, tree fntype)
{
  ...
  cum->gp_reg_found = cum->stdarg = (cum->prototype && stdarg_p (fntype));
}
		   ...
		   machine_mode mode, const_tree type, bool named)
{
  ...
  if (TARGET_MXU3 && TARGET_MXU3_EXT_ABI
        && MXU3_SUPPORTED_MODE_P (mode) && (mode != V1SImode)
        && named && !cum->stdarg /* ignore the variable arguments */)
    {
      info->fpr_p = false;
      info->vpr_p = true;
      info->reg_offset = cum->num_vprs;
      /* Align to 16-words boundary */
      info->stack_offset = ROUND_UP (cum->stack_words, 16);
...
mips_arg_regno (const struct mips_arg_info *info, bool hard_float_p)
{
  if (info->vpr_p)
    {
      gcc_assert (TARGET_MXU3 && TARGET_MXU3_EXT_ABI);
      return VP_ARG_FIRST + info->reg_offset;
    }
		   ...
		   const_tree type, bool named)
{
  ...
  /* We will not pass V1SI mode values via registers, 
     this will cause crash during the LRA pass. */
  if (TARGET_MXU3 && mode == V1SImode)
    return NULL;
			   ...
			   const_tree type, bool named)
{
  ...
  if (info.vpr_p && info.reg_words > 0)
    cum->num_vprs++;
...

/* See whether VALTYPE is a record 
 */
static int
mips_mxu3_vpr_return_fields (const_tree valtype, machine_mode *modes)
{
  tree field;
  int i, j;

  if (!TARGET_MXU3_EXT_ABI || TREE_CODE (valtype) != RECORD_TYPE)
    return 0;
  ...
  continue;
      ...
      if (TREE_CODE (TREE_TYPE (field)) == ARRAY_TYPE
          && VECTOR_TYPE_P (TREE_TYPE (TREE_TYPE (field)))) 
      {
        tree eletype = TREE_TYPE (TREE_TYPE (field));
        tree size_tree;
        HOST_WIDE_INT size = -1;
        
        mode = TYPE_MODE (eletype);
        if (!MXU3_SUPPORTED_MODE_P (mode) || mode == V1SImode)
          return 0;

        size_tree = TYPE_SIZE (TREE_TYPE (field));
        if (size_tree == NULL_TREE || !tree_fits_uhwi_p (size_tree))
          return 0;

        size = tree_to_uhwi (size_tree) / GET_MODE_BITSIZE (mode);
...
static rtx
mips_mxu3_return_vpr_aggregate (machine_mode mode, 
          machine_mode *modes, int n) {
  gcc_assert (n > 0 && n <= 4 && TARGET_MXU3_EXT_ABI);

  unsigned long regno = VP_RETURN, offset = 0;
  rtx par[4];
  int i;

  for (i = 0; i < n; ++i)
    {
      par[i] = gen_rtx_EXPR_LIST (VOIDmode,
                  gen_rtx_REG (modes[i], regno++),
                  GEN_INT (offset));
      offset += GET_MODE_SIZE (modes[i]);
    }

  return gen_rtx_PARALLEL (mode, gen_rtvec_v (n, par));
}
		       ...
		       machine_mode mode)
{
  if (valtype)
	...
	case 2:
      ...
      /* Handle structures whose fields are returned in $vpr0~$vpr3. */
      if (TARGET_MXU3 && TARGET_MXU3_EXT_ABI)
  {
    int n = -1;
    machine_mode modes[4];
    
    if ((n = mips_mxu3_vpr_return_fields (valtype, modes)) > 1)
      return mips_mxu3_return_vpr_aggregate (mode, modes, n);
  }
  ...
  if (TARGET_MXU3 && TARGET_MXU3_EXT_ABI)
    {
      if (MXU3_SUPPORTED_MODE_P (mode) && mode != V1SImode)
        return gen_rtx_REG (mode, VP_RETURN);
    }
  return gen_rtx_REG (mode, GP_RETURN);
}
...
mips_function_value_regno_p (const unsigned int regno)
{
  ...
  /* Use the first COP2 register for return vector values
     when MXU3 extended SIMD ABI is enabled. */
  if (TARGET_MXU3 && TARGET_MXU3_EXT_ABI
      && (regno >= VP_RETURN && regno < VP_RETURN + 4))
    return true;
...
{
  /* Make sure the V1SImode value must be returned via the stack. */
  if (TARGET_MXU3 && TYPE_MODE (type) == V1SImode)
    return true;

  /* The 512-bit vector types are returned via VPR if the MXU3 extended
     SIMD ABI is enabled. */
  if (TARGET_MXU3 && TARGET_MXU3_EXT_ABI)
    {
      machine_mode modes[4];

      if (MXU3_SUPPORTED_MODE_P (TYPE_MODE (type)))
        return false;
      
      if (mips_mxu3_vpr_return_fields (type, modes) > 1)
        return false;
    }
  ...
  if (result == 0)
	...
	fn = gen_call_internal;
      ...
      pattern = fn (addr, args_size);
           ...
           && (nret = XVECLEN (result, 0)) >= 2)
    {
      if (nret == 2)
        {
      /* Handle return values created by mips_return_fpr_pair
         or mips_mxu3_return_vpr_aggregate. */
	...
	fn = gen_call_value_multiple_internal;
      ...
      pattern = fn (reg1, addr, args_size, reg2);
        ...
        {
          /* Handle return values created by mips_mxu3_return_vpr_aggregate. */
          rtx regs[4];

          gcc_assert (nret == 4 || nret == 3);
          ...
          if (nret == 3)
            {
              if (type == MIPS_CALL_SIBCALL)
        pattern = gen_sibcall_value_triple_internal (regs[0], 
                      addr, args_size, regs[1], regs[2]);
              else
        pattern = gen_call_value_triple_internal (regs[0], 
                      addr, args_size, regs[1], regs[2]);
            }
          else
            {
              if (type == MIPS_CALL_SIBCALL)
        pattern = gen_sibcall_value_quadruple_internal (regs[0],
                      addr, args_size, regs[1], regs[2], regs[3]);
              else
        pattern = gen_call_value_quadruple_internal (regs[0],
                      addr, args_size, regs[1], regs[2], regs[3]);
            }
        }
    }
...
/* Print the text for PRINT_OPERAND punctation character CH to FILE.
   ...
   '!'  Print "s" to use the short version if the delay slot contains a
	16-bit instruction.
   '&'  Print MSA and MXU2 different instructions format.
...
/* Implement TARGET_PRINT_OPERAND.  The MIPS-specific operand codes are:
   ...
   'e'	Print a MXU3 VWR register value.
   'X'	Print CONST_INT OP in hexadecimal format.
   'x'	Print the low 16 bits of CONST_INT OP in hexadecimal format.
   'd'	Print CONST_INT OP in decimal.
   'B'	Print CONST_INT OP element 0 of a replicated CONST_VECTOR
	  as an unsigned byte [0..255].
   'H'	Print CONST_INT OP element 0 of a replicated CONST_VECTOR
	  as an unsigned byte [0..65535].
   'Q'	Print CONST_INT OP element 0 of a replicated CONST_VECTOR
	  as an unsigned byte [0..255].
   ...
   'w'	Print a MSA and MXU2 register.
   ...
   'V'	Print exact log2 of CONST_INT OP element 0 of a replicated CONST_VECTOR
          in decimal for MSA. Print the insn size suffix 16b,8h,4w,2d for MXU2
          vector cond branch modes V16QI,V8HI,V4SI,V2DI.
   '?'  Print dot.  */
...
mips_print_operand (FILE *file, rtx op, int letter)
{
  ...
  switch (letter)
      ...
      if (GET_CODE (op) == CONST_VECTOR)
	{
	  ...
	  short val = (short)(UINTVAL (op) & 0xffff);
      ...
      if (GET_CODE (op) == CONST_VECTOR)
	{
	  gcc_assert (mips_const_vector_same_val_p (op, GET_MODE (op)));
	  op = CONST_VECTOR_ELT (op, 0);
	  unsigned HOST_WIDE_INT val = (UINTVAL (op) >> 16) & 0xffff;
	  gcc_assert (CONST_INT_P (op));
      ...
      if (GET_CODE (op) == CONST_VECTOR)
	{
	  ...
	  op = CONST_VECTOR_ELT (op, 0);
	  gcc_assert (CONST_INT_P (op));
	  unsigned HOST_WIDE_INT val16 = UINTVAL (op) & GET_MODE_MASK (HImode);
	  fprintf (file, HOST_WIDE_INT_PRINT_UNSIGNED, val16);
	}
      else
	output_operand_lossage ("invalid use of '%%%c'", letter);
      break;
      ...
      if (GET_CODE (op) == CONST_VECTOR)
	{
	  ...
	  gcc_assert (CONST_INT_P (op));
	  unsigned HOST_WIDE_INT val8 = UINTVAL (op) & GET_MODE_MASK (QImode);
	  fprintf (file, HOST_WIDE_INT_PRINT_UNSIGNED, ((val8 << 8) | val8));
	}
      else
	output_operand_lossage ("invalid use of '%%%c'", letter);
      break;
	...
	fprintf (file, "$w%s", &reg_names[REGNO (op)][2]);
      else if (code == REG && (MXU2_REG_P (REGNO (op)) || MXU3_REG_P (REGNO (op))))
	fprintf (file, "$v%s", &reg_names[REGNO (op)][2]);
      else if (code == REG && VSR_REG_P (REGNO (op)))
	fprintf (file, "$v%s", &reg_names[REGNO (op)][2]);
      else if (code == REG && VWR_REG_P (REGNO (op)))
	fprintf (file, "$v%s", &reg_names[REGNO (op)][2]);
      else
	output_operand_lossage ("invalid use of '%%%c'", letter);
      break;
    ...
    case 'e':
      if (code == REG && VWR_REG_P (REGNO (op)))
...
mips_file_start (void)
{
  ...
  /* If ISA_HAS_MXU3, tell GAS wheather using MXU3 EXT ABI or not. */
  if (TARGET_MXU3_EXT_ABI)
    fputs ("\t.module\tmxu3extabi\n", asm_out_file);
...
#ifdef HAVE_AS_GNU_ATTRIBUTE
  {
    ...
    /* 128-bit MSA and MXU2.  */
    if (TARGET_MSA || TARGET_MXU2)
...
mips_compute_frame_info (void)
{
  ...
  /* Find out which VPRs and VWRs we need to save. */ 
  if (ISA_HAS_MXU3 && TARGET_MXU3_EXT_ABI)
    {
      /* Check VPRs. */
      for (regno = COP2_REG_FIRST; regno <= COP2_REG_LAST; regno++)
        if (mips_save_reg_p (regno))
          {
        frame->num_cop2_regs++;
        frame->cop2_mask |= (1U << (regno - COP2_REG_FIRST));
          }

      if (frame->num_cop2_regs > 0)
        {
          offset += MIPS_STACK_ALIGN (frame->num_cop2_regs * UNITS_PER_MXU3_REG);
          frame->cop2_sp_offset = offset - UNITS_PER_MXU3_REG;
        }

      /* Check VWRs. */
      for (regno = VWR_REG_FIRST; regno <= VWR_REG_LAST; regno++)
        if (mips_save_reg_p (regno))
          {
        frame->num_vwr_regs++;
        frame->vwr_mask |= (1U << (regno - VWR_REG_FIRST));
          }

      if (frame->num_vwr_regs > 0)
        {
          offset += MIPS_STACK_ALIGN (frame->num_vwr_regs * UNITS_PER_WORD);
          frame->vwr_sp_offset = offset - UNITS_PER_WORD;
        }
    }
  ...
  if (ISA_HAS_MXU3 && TARGET_MXU3_EXT_ABI)
    {
      if (frame->num_cop2_regs > 0)
        frame->cop2_save_offset = frame->cop2_sp_offset - offset;
      if (frame->num_vwr_regs > 0)
        frame->vwr_save_offset = frame->vwr_sp_offset - offset;
    }
}
...
/* Call FN for each VPR and VWR register that is saved by the current
   function. SP_OFFSET is the offset of the current stack pointer from
   the start of the frame. */
static void
mips_for_each_saved_vpr_and_wpr (HOST_WIDE_INT sp_offset,
          mips_save_restore_fn fn)
{
  int regno;
  const struct mips_frame_info *frame = &cfun->machine->frame;
  ...
  gcc_assert (ISA_HAS_MXU3 && TARGET_MXU3_EXT_ABI);

  offset = frame->cop2_sp_offset - sp_offset;
  mask = frame->cop2_mask;
  
  for (regno = COP2_REG_LAST; regno >= COP2_REG_FIRST; regno--)
    if (BITSET_P (mask, regno - COP2_REG_FIRST))
      {
    mips_save_restore_reg (V16SImode, regno, offset, fn);
    offset -= UNITS_PER_MXU3_REG;
      }

  offset = frame->vwr_sp_offset - sp_offset;
  mask = frame->vwr_mask;

  for (regno = VWR_REG_LAST; regno >= VWR_REG_FIRST; regno--)
    if (BITSET_P (mask, regno - VWR_REG_FIRST))
      {
    mips_save_restore_reg (V1SImode, regno, offset, fn);
    offset -= UNITS_PER_WORD;
      }
...
mips_emit_save_slot_move (rtx dest, rtx src, rtx temp)
{
  ...
  if (regno == HI_REGNUM)
  ...
  else if (mips_direct_save_slot_move_p (regno, mem, mem == src))
    {
      if (VWR_REG_P (regno))
        emit_insn (gen_mxu3_vwr_move (dest, src));
      else if (COP2_REG_P (regno))
        {
          if (GET_CODE(XEXP (mem, 0)) == PLUS)
          {
            rtx addr, offset, new_mem;
            rtx_insn* insn;

            gcc_assert ((GET_CODE (XEXP (XEXP (mem, 0), 0)) == REG) 
                        && (GET_CODE (XEXP (XEXP (mem, 0), 1)) == CONST_INT));
            offset = XEXP (XEXP (mem, 0), 1);

            if (!mips_signed_immediate_p (INTVAL (offset) + 32, 5,
                  mips_ldst_scaled_shift (OImode)))
            {
              addr = gen_rtx_REG (SImode, REGNO (temp));
              insn = emit_move_insn (addr, XEXP (mem, 0));
          ...
          {
            rtx x1, x2;
            rtx new_dest, new_src;

            new_dest = mips_mxu3_simplify_subreg (
                OImode, dest, GET_MODE (dest), 0);
            new_src  = simplify_subreg (V2OImode, src, GET_MODE (src), 0);

            x1 = mips_frame_set (new_dest, new_src);

            new_dest = mips_mxu3_simplify_subreg (
                OImode, dest, GET_MODE (dest), 32);
            
            x2 = mips_frame_set (new_dest, new_src);
...
mips_expand_prologue (void)
{
  ...
  if (((frame->mask | frame->fmask | frame->acc_mask) != 0)
      || frame->num_cop0_regs > 0
      || (frame->cop2_mask | frame->vwr_mask) != 0)
    {
      HOST_WIDE_INT step1;
    ...
    if (ISA_HAS_MXU3 && TARGET_MXU3_EXT_ABI)
      mips_for_each_saved_vpr_and_wpr (size, mips_save_reg);
	}
    }
...
mips_expand_epilogue (bool sibcall_p)
{
  ...
  if (GENERATE_MIPS16E_SAVE_RESTORE && frame->mask != 0)
  ...
  else
    {
      ...
      if (ISA_HAS_MXU3 && TARGET_MXU3_EXT_ABI)
        mips_for_each_saved_vpr_and_wpr (frame->total_size - step2,
                  mips_restore_reg);
...
mips_hard_regno_mode_ok_p (unsigned int regno, machine_mode mode)
{
  ...
  /* For MXU2, allow TImode and 128-bit vector modes in all MXU2_REG.  */
  if (MXU2_REG_P (regno) && MXU2_SUPPORTED_MODE_P (mode))
    return true;

  /* For MXU3, allow V1SImode mode for VWR_REG and MXU3_REG only.  */
  if (ISA_HAS_MXU3 && (mode == V1SImode))
    {
      if (VWR_REG_P (regno) || MXU3_REG_P (regno))
        return true;
      else
        return false;
    }

  /* For MXU3, allow TImode and 512-bit vector modes in all MXU3_REG.  */
  if ((MXU3_REG_P (regno) || VSR_REG_P (regno)) && MXU3_SUPPORTED_MODE_P (mode))
    return true;
  
  /* For MXU3, allow SImode in MXU3_REG. */
  if (MXU3_REG_P (regno) && mode == SImode)
    return true;
...
mips_hard_regno_nregs (int regno, machine_mode mode)
{
  ...
  if ((MXU3_REG_P (regno) || VSR_REG_P (regno) || VWR_REG_P (regno))
      && MXU3_SUPPORTED_MODE_P(mode))
    return 1;
...
mips_class_max_nregs (enum reg_class rclass, machine_mode mode)
{
  ...
  if (hard_reg_set_intersect_p (left, reg_class_contents[(int) COP2_REGS]))
    {
      if (HARD_REGNO_MODE_OK (COP2_REG_FIRST, mode)
	  && MXU2_SUPPORTED_MODE_P (mode))
	{
	  size = MIN (size, UNITS_PER_MXU2_REG);
	  AND_COMPL_HARD_REG_SET (left, reg_class_contents[(int) COP2_REGS]);
	}

      if (HARD_REGNO_MODE_OK (COP2_REG_FIRST, mode)
	  && MXU3_SUPPORTED_MODE_P (mode))
	{
	  size = MIN (size, UNITS_PER_MXU3_REG);
	  AND_COMPL_HARD_REG_SET (left, reg_class_contents[(int) COP2_REGS]);
	}
    }

  if (hard_reg_set_intersect_p (left, reg_class_contents[(int) VSR_REGS]))
    {
      if (HARD_REGNO_MODE_OK (VSR_REG_FIRST, mode)
	  && MXU3_SUPPORTED_MODE_P (mode))
	{
	  size = MIN (size, UNITS_PER_MXU3_REG);
	  AND_COMPL_HARD_REG_SET (left, reg_class_contents[(int) VSR_REGS]);
	}
    }

  if (hard_reg_set_intersect_p (left, reg_class_contents[(int) VWR_REGS]))
    {
      if (HARD_REGNO_MODE_OK (VWR_REG_FIRST, mode) && (mode == V1SImode))
	{
	  size = MIN (size, UNITS_PER_MXU3_REG);
	  AND_COMPL_HARD_REG_SET (left, reg_class_contents[(int) VWR_REGS]);
	}
    }
			       ...
			       enum reg_class rclass)
{
  ...
  /* Allow conversions between different MXU3 vector modes.  */
  if (MXU3_SUPPORTED_MODE_P (from) && MXU3_SUPPORTED_MODE_P (to))
    return false;
...
mips_preferred_reload_class (rtx x, reg_class_t rclass)
{
  ...
  /* Deal for MXU2.  */
  if (TARGET_MXU2
      && reg_class_subset_p (COP2_REGS, rclass)
			 ...
			 reg_class_t from, reg_class_t to)
{
  ...
  if (ISA_HAS_MXU3) 
  {
    if ((from == COP2_REGS && to == VSR_REGS) 
        || (from == VSR_REGS && to == COP2_REGS))
      return 8;

    if ((from == COP2_REGS && to == VWR_REGS) 
        || (from == VWR_REGS && to == COP2_REGS))
      return 9;
  }
			      ...
			      machine_mode mode)
{
  ...
  if (ISA_HAS_MXU3
      && ((class1 == COP2_REGS || class1 == VSR_REGS
          || class1 == VWR_REGS) != (class2 == COP2_REGS
          || class2 == VSR_REGS || class2 == VWR_REGS)))
    return true;
...
mips_preferred_simd_mode (machine_mode mode)
{
  ...
  if (!ISA_HAS_MSA && !ISA_HAS_MXU2 && !ISA_HAS_MXU3)
    return word_mode;
...
static bool
mips_mxu3_simd_branch_p (rtx_insn *insn)
{
  return ISA_HAS_MXU3 && get_attr_type (insn) == TYPE_SIMD_BRANCH;
}

#define MXU3_CONDBRANCH_MIN -1024
#define MXU3_CONDBRANCH_MAX 1020
  ...
  if (length <= 8)
    {
      ...
      if (mips_mxu3_simd_branch_p (insn))
        {
          int distance = INSN_ADDRESSES (INSN_UID (insn))
                       - INSN_ADDRESSES (INSN_UID (operands[0])) + 4;
          return_p = distance >= MXU3_CONDBRANCH_MIN
                  && distance <= MXU3_CONDBRANCH_MAX;
        }
...
mips_mxu2_output_division (const char *division, rtx *operands)
{
  ...
  if (TARGET_CHECK_ZERO_DIV)
    {
      output_asm_insn ("%(bnez%V0\t%w2,1f", operands);
      output_asm_insn (s, operands);
      s = "break\t7%)\n1:";
    }
  return s;
...
static int
mips_adjust_cost (rtx_insn *insn, int dep_type, rtx_insn *dep, int cost, unsigned int)
{
  ...
  /* For ingenic mxu2 */
  rtx link;
  if ((ISA_HAS_MXU2 || ISA_HAS_MXU3)
      && recog_memoized (insn) >= 0
      && recog_memoized (dep) >= 0
      && ((enum attr_lat_type)get_attr_lat_type(dep) != LAT_TYPE_UNKNOWN))
    return mips_adjust_cost_ingenic(insn, link, dep, cost);
...
AVAIL_NON_MIPS16 (mxu3, TARGET_MXU3)
AVAIL_NON_MIPS16 (mxu3_only,TARGET_MXU3 && !TARGET_MXU3_1)
AVAIL_NON_MIPS16 (mxu3_1, TARGET_MXU3_1)
...
#define CODE_FOR_msa_fmadd_w CODE_FOR_fmav4sf4_intrinsic
#define CODE_FOR_msa_fmadd_d CODE_FOR_fmav2df4_intrinsic
#define CODE_FOR_msa_fmsub_w CODE_FOR_fnmav4sf4_intrinsic
#define CODE_FOR_msa_fmsub_d CODE_FOR_fnmav2df4_intrinsic
...
/* Ingenic Xburst2 SIMD extension MXA Instructions. */
#define CODE_FOR_msa_bmr_v CODE_FOR_msa_bmr_b
...
#define MIPS_SET_FCSR 1
  ...
  /* Ingenic Xburst2 SIMD extension MXA instructions. */
  MSA_BUILTIN (fcmul_w, MIPS_V4SF_FTYPE_V4SF_V4SF),
  MSA_BUILTIN (fexuplc_w, MIPS_V4SF_FTYPE_V8HI_V8HI),
  MSA_BUILTIN (fexuprc_w, MIPS_V4SF_FTYPE_V8HI_V8HI),
  MSA_BUILTIN (ldinss_w, MIPS_V4SI_FTYPE_V4SI_UQI_POINTER_SI),
  MSA_BUILTIN (ldins_w, MIPS_V4SI_FTYPE_V4SI_UQI_POINTER_SI),
  MSA_BUILTIN (ldins_d, MIPS_V2DI_FTYPE_V2DI_UQI_POINTER_SI),
  MSA_NO_TARGET_BUILTIN (stext_w, MIPS_VOID_FTYPE_V4SI_UQI_POINTER_SI),
  MSA_NO_TARGET_BUILTIN (stext_d, MIPS_VOID_FTYPE_V2DI_UQI_POINTER_SI),
  MSA_BUILTIN (vshfr_b, MIPS_V16QI_FTYPE_V16QI_V16QI_V16QI),
  MSA_BUILTIN (vsldi_b, MIPS_V16QI_FTYPE_V16QI_V16QI_UQI),
  MSA_BUILTIN (bmr_v, MIPS_V16QI_FTYPE_V16QI_V16QI_V16QI),
  MSA_BUILTIN (pcnt_v, MIPS_V16QI_FTYPE_V16QI),
  MSA_BUILTIN (bext_b, MIPS_V16QI_FTYPE_V16QI),
  MSA_BUILTIN (bext_h, MIPS_V8HI_FTYPE_V8HI),
  MSA_BUILTIN (bext_w, MIPS_V4SI_FTYPE_V4SI),
  MSA_BUILTIN (bexp_b, MIPS_V16QI_FTYPE_V16QI),
  MSA_BUILTIN (bexp_h, MIPS_V8HI_FTYPE_V8HI),
  MSA_BUILTIN (bexp_w, MIPS_V4SI_FTYPE_V4SI),
  MSA_BUILTIN (bexp_2b, MIPS_V16QI_FTYPE_V16QI),
  MSA_BUILTIN (bexp_4b, MIPS_V16QI_FTYPE_V16QI),
  MSA_BUILTIN (bmu_v, MIPS_V16QI_FTYPE_V16QI_V16QI_V16QI),
  MSA_BUILTIN (sll_v, MIPS_V16QI_FTYPE_V16QI_V16QI),
  MSA_BUILTIN (slli_v, MIPS_V16QI_FTYPE_V16QI_UQI),
  MSA_BUILTIN (srl_v, MIPS_V16QI_FTYPE_V16QI_V16QI),
  MSA_BUILTIN (srli_v, MIPS_V16QI_FTYPE_V16QI_UQI),
  MSA_BUILTIN (move_w, MIPS_V4SI_FTYPE_V4SI_UQI_V4SI_UQI),
  MSA_BUILTIN (movbt_b, MIPS_V16QI_FTYPE_V16QI_V16QI),
  MSA_BUILTIN (movbp_b, MIPS_V16QI_FTYPE_V16QI_V16QI),
  MSA_BUILTIN (satss_b, MIPS_V16QI_FTYPE_V8HI_V8HI),
  MSA_BUILTIN (satss_h, MIPS_V8HI_FTYPE_V4SI_V4SI),
  MSA_BUILTIN (satuu_b, MIPS_UV16QI_FTYPE_UV8HI_UV8HI),
  MSA_BUILTIN (satuu_h, MIPS_UV8HI_FTYPE_UV4SI_UV4SI),
  MSA_BUILTIN (satus_b, MIPS_UV16QI_FTYPE_V8HI_V8HI),
  MSA_BUILTIN (satus_h, MIPS_UV8HI_FTYPE_V4SI_V4SI),
  MSA_BUILTIN (addsl_h, MIPS_V8HI_FTYPE_V16QI_V16QI),
  MSA_BUILTIN (addsl_w, MIPS_V4SI_FTYPE_V8HI_V8HI),
  MSA_BUILTIN (addsr_h, MIPS_V8HI_FTYPE_V16QI_V16QI),
  MSA_BUILTIN (addsr_w, MIPS_V4SI_FTYPE_V8HI_V8HI),
  MSA_BUILTIN (addul_h, MIPS_UV8HI_FTYPE_UV16QI_UV16QI),
  MSA_BUILTIN (addul_w, MIPS_UV4SI_FTYPE_UV8HI_UV8HI),
  MSA_BUILTIN (addur_h, MIPS_UV8HI_FTYPE_UV16QI_UV16QI),
  MSA_BUILTIN (addur_w, MIPS_UV4SI_FTYPE_UV8HI_UV8HI),
  MSA_BUILTIN (accsl_h, MIPS_V8HI_FTYPE_V8HI_V16QI),
  MSA_BUILTIN (accsl_w, MIPS_V4SI_FTYPE_V4SI_V8HI),
  MSA_BUILTIN (accsr_h, MIPS_V8HI_FTYPE_V8HI_V16QI),
  MSA_BUILTIN (accsr_w, MIPS_V4SI_FTYPE_V4SI_V8HI),
  MSA_BUILTIN (accul_h, MIPS_UV8HI_FTYPE_UV8HI_UV16QI),
  MSA_BUILTIN (accul_w, MIPS_UV4SI_FTYPE_UV4SI_UV8HI),
  MSA_BUILTIN (accur_h, MIPS_UV8HI_FTYPE_UV8HI_UV16QI),
  MSA_BUILTIN (accur_w, MIPS_UV4SI_FTYPE_UV4SI_UV8HI),
  MSA_BUILTIN (subsl_h, MIPS_V8HI_FTYPE_V16QI_V16QI),
  MSA_BUILTIN (subsl_w, MIPS_V4SI_FTYPE_V8HI_V8HI),
  MSA_BUILTIN (subsr_h, MIPS_V8HI_FTYPE_V16QI_V16QI),
  MSA_BUILTIN (subsr_w, MIPS_V4SI_FTYPE_V8HI_V8HI),
  MSA_BUILTIN (subul_h, MIPS_V8HI_FTYPE_UV16QI_UV16QI),
  MSA_BUILTIN (subul_w, MIPS_V4SI_FTYPE_UV8HI_UV8HI),
  MSA_BUILTIN (subur_h, MIPS_V8HI_FTYPE_UV16QI_UV16QI),
  MSA_BUILTIN (subur_w, MIPS_V4SI_FTYPE_UV8HI_UV8HI),
  MSA_BUILTIN (mulsl_h, MIPS_V8HI_FTYPE_V16QI_V16QI),
  MSA_BUILTIN (mulsl_w, MIPS_V4SI_FTYPE_V8HI_V8HI),
  MSA_BUILTIN (mulsr_h, MIPS_V8HI_FTYPE_V16QI_V16QI),
  MSA_BUILTIN (mulsr_w, MIPS_V4SI_FTYPE_V8HI_V8HI),
  MSA_BUILTIN (mulul_h, MIPS_UV8HI_FTYPE_UV16QI_UV16QI),
  MSA_BUILTIN (mulul_w, MIPS_UV4SI_FTYPE_UV8HI_UV8HI),
  MSA_BUILTIN (mulur_h, MIPS_UV8HI_FTYPE_UV16QI_UV16QI),
  MSA_BUILTIN (mulur_w, MIPS_UV4SI_FTYPE_UV8HI_UV8HI),
#include "ingenic-mxu2.def"
#include "ingenic-mxu3.def"
#include "ingenic-mxu3.1.def"
};
...
#define MIPS_ATYPE_V4TI						\
  mips_builtin_vector_type (intTI_type_node, V4TImode)

#define MIPS_ATYPE_UV4TI					\
  mips_builtin_vector_type (unsigned_intTI_type_node, V4TImode)

#define MIPS_ATYPE_V2DI						\
  mips_builtin_vector_type (long_long_integer_type_node, V2DImode)
#define MIPS_ATYPE_V8DI						\
  mips_builtin_vector_type (long_long_integer_type_node, V8DImode)
#define MIPS_ATYPE_UV8DI					\
  mips_builtin_vector_type (long_long_unsigned_type_node, V8DImode)
#define MIPS_ATYPE_V1SI mips_builtin_vector_type (intSI_type_node, V1SImode)
#define MIPS_ATYPE_V4SI mips_builtin_vector_type (intSI_type_node, V4SImode)
#define MIPS_ATYPE_V16SI mips_builtin_vector_type (intSI_type_node, V16SImode)
#define MIPS_ATYPE_V8HI mips_builtin_vector_type (intHI_type_node, V8HImode)
#define MIPS_ATYPE_V32HI mips_builtin_vector_type (intHI_type_node, V32HImode)
#define MIPS_ATYPE_V16QI mips_builtin_vector_type (intQI_type_node, V16QImode)
#define MIPS_ATYPE_V64QI mips_builtin_vector_type (intQI_type_node, V64QImode)
...
#define MIPS_ATYPE_V16SF mips_builtin_vector_type (float_type_node, V16SFmode)
...
#define MIPS_ATYPE_UV16SI					\
  mips_builtin_vector_type (unsigned_intSI_type_node, V16SImode)
#define MIPS_ATYPE_UV8HI					\
  mips_builtin_vector_type (unsigned_intHI_type_node, V8HImode)
#define MIPS_ATYPE_UV32HI					\
...
#define MIPS_ATYPE_UV64QI					\
  mips_builtin_vector_type (unsigned_intQI_type_node, V64QImode)
...
/* For Ingenic MXU3 */
#define MIPS_ATYPE_UV2OI					\
  mips_builtin_vector_type (mips_mxu3_unsigned_intOI_type_node, V2OImode)
...
mips_init_builtins (void)
{
  ...
  if (ISA_HAS_MXU3)
    {
      mips_mxu3_unsigned_intOI_type_node = 
        make_unsigned_type (GET_MODE_BITSIZE (OImode));

      #define MIPS_MXU3_ADD_BUILTIN(mode, builtin_type) \
        add_builtin_type (#builtin_type, MIPS_ATYPE_##mode)
      
      MIPS_MXU3_ADD_BUILTIN (V16SF, __builtin_mxu3_v16f32);
    
      MIPS_MXU3_ADD_BUILTIN (UV64QI, __builtin_mxu3_v64u8);
      MIPS_MXU3_ADD_BUILTIN (UV32HI, __builtin_mxu3_v32u16);
      MIPS_MXU3_ADD_BUILTIN (UV16SI, __builtin_mxu3_v16u32);
      MIPS_MXU3_ADD_BUILTIN (UV8DI, __builtin_mxu3_v8u64);
      MIPS_MXU3_ADD_BUILTIN (UV4TI, __builtin_mxu3_v4u128);
      MIPS_MXU3_ADD_BUILTIN (UV2OI, __builtin_mxu3_v2u256);

      MIPS_MXU3_ADD_BUILTIN (V64QI, __builtin_mxu3_v64i8);
      MIPS_MXU3_ADD_BUILTIN (V32HI, __builtin_mxu3_v32i16);
      MIPS_MXU3_ADD_BUILTIN (V16SI, __builtin_mxu3_v16i32);
      MIPS_MXU3_ADD_BUILTIN (V8DI, __builtin_mxu3_v8i64);
      MIPS_MXU3_ADD_BUILTIN (V4TI, __builtin_mxu3_v4i128);

      MIPS_MXU3_ADD_BUILTIN (V1SI, __builtin_mxu3_v1i32);

      #undef MIPS_MXU3_ADD_BUILTIN
    }
...
mips_builtin_vectorized_function (unsigned int fn, tree type_out, tree type_in)
{
  ...
  if (TREE_CODE (type_out) != VECTOR_TYPE
      ...
      || (!ISA_HAS_MSA && !ISA_HAS_MXU2 && !ISA_HAS_MXU3))
    return NULL_TREE;

  gcc_assert(!(ISA_HAS_MSA && ISA_HAS_MXU2 && !ISA_HAS_MXU3));
...
#define MIPS_GET_BUILTIN(INSN) \
  ...
  switch (fn)
	...
	return ISA_HAS_MSA ? MIPS_GET_BUILTIN (msa_fsqrt_d) :
	  MIPS_GET_BUILTIN (mxu2_fsqrt_d);
      break;
    case BUILT_IN_SQRTF:
      if (out_mode == SFmode && out_n == 4
	  && in_mode == SFmode && in_n == 4)
	return ISA_HAS_MSA ? MIPS_GET_BUILTIN (msa_fsqrt_w) :
	  MIPS_GET_BUILTIN (mxu2_fsqrt_w);
      break;
    default:
      break;
    }
			  ...
			  struct expand_operand *ops, bool has_target_p)
{
  ...
  esize = 1; /* for GT<n><fmt> insn (ingenic, mxu3). */
  elemsize = 64; /* for REPI(B,H,W,D,Q,X). BSH(L,R)I, S*SUM LOAD-SCATTER STORE-GATHER*/
  switch (icode)
    {
    ...
    case CODE_FOR_mxu3_andib:
    case CODE_FOR_mxu3_orib:
    case CODE_FOR_mxu3_xorib:
      if (CONST_INT_P (ops[2].value))
	{
	  rangelo = 0;
	  rangehi = 255;
    ...
    case CODE_FOR_mxu3_sr1mac2bi:
    case CODE_FOR_mxu3_sr1mac4bi:
    case CODE_FOR_mxu3_sr1macuub:
    case CODE_FOR_mxu3_sr1macsub:
    case CODE_FOR_mxu3_sr1macssb:
    case CODE_FOR_mxu3_sr1macssh:
    case CODE_FOR_mxu3_s1macuub:
    case CODE_FOR_mxu3_s1macsub:
    case CODE_FOR_mxu3_s1macssb:
    case CODE_FOR_mxu3_s1macssh:
    case CODE_FOR_mxu3_sr1sum2bi:
    case CODE_FOR_mxu3_sr1sum4bi:
    case CODE_FOR_mxu3_sr1sumub:
    case CODE_FOR_mxu3_sr1sumuh:
    case CODE_FOR_mxu3_sr1sumsb:
    case CODE_FOR_mxu3_sr1sumsh:
    case CODE_FOR_mxu3_sr1sumw:
      elemsize = elemsize >> 1;

    case CODE_FOR_mxu3_sr2mac2bi:
    case CODE_FOR_mxu3_sr2mac4bi:
    case CODE_FOR_mxu3_sr2macuub:
    case CODE_FOR_mxu3_sr2macsub:
    case CODE_FOR_mxu3_sr2macssb:
    case CODE_FOR_mxu3_sr2macssh:
    case CODE_FOR_mxu3_s2macuub:
    case CODE_FOR_mxu3_s2macsub:
    case CODE_FOR_mxu3_s2macssb:
    case CODE_FOR_mxu3_s2macssh:
    case CODE_FOR_mxu3_sr2sum2bi:
    case CODE_FOR_mxu3_sr2sum4bi:
    case CODE_FOR_mxu3_sr2sumub:
    case CODE_FOR_mxu3_sr2sumuh:
    case CODE_FOR_mxu3_sr2sumsb:
    case CODE_FOR_mxu3_sr2sumsh:
    case CODE_FOR_mxu3_sr2sumw:
    case CODE_FOR_mxu3_repio:
      elemsize = elemsize >> 1;

    case CODE_FOR_mxu3_sr4mac2bi:
    case CODE_FOR_mxu3_sr4mac4bi:
    case CODE_FOR_mxu3_sr4macuub:
    case CODE_FOR_mxu3_sr4macsub:
    case CODE_FOR_mxu3_sr4macssb:
    case CODE_FOR_mxu3_sr4macssh:
    case CODE_FOR_mxu3_s4macuub:
    case CODE_FOR_mxu3_s4macsub:
    case CODE_FOR_mxu3_s4macssb:
    case CODE_FOR_mxu3_s4macssh:
    case CODE_FOR_mxu3_sr4sum2bi:
    case CODE_FOR_mxu3_sr4sum4bi:
    case CODE_FOR_mxu3_sr4sumub:
    case CODE_FOR_mxu3_sr4sumuh:
    case CODE_FOR_mxu3_sr4sumsb:
    case CODE_FOR_mxu3_sr4sumsh:
    case CODE_FOR_mxu3_sr4sumw:
    case CODE_FOR_mxu3_repiq:
      elemsize = elemsize >> 1;

    case CODE_FOR_mxu3_sr8mac2bi:
    case CODE_FOR_mxu3_sr8mac4bi:
    case CODE_FOR_mxu3_sr8macuub:
    case CODE_FOR_mxu3_sr8macsub:
    case CODE_FOR_mxu3_sr8macssb:
    case CODE_FOR_mxu3_sr8macssh:
    case CODE_FOR_mxu3_s8macuub:
    case CODE_FOR_mxu3_s8macsub:
    case CODE_FOR_mxu3_s8macssb:
    case CODE_FOR_mxu3_s8macssh:
    case CODE_FOR_mxu3_sr8sum2bi:
    case CODE_FOR_mxu3_sr8sum4bi:
    case CODE_FOR_mxu3_sr8sumub:
    case CODE_FOR_mxu3_sr8sumuh:
    case CODE_FOR_mxu3_sr8sumsb:
    case CODE_FOR_mxu3_sr8sumsh:
    case CODE_FOR_mxu3_sr8sumw:
    case CODE_FOR_mxu3_repid:
      elemsize = elemsize >> 1;

    case CODE_FOR_mxu3_sr16mac2bi:
    case CODE_FOR_mxu3_sr16mac4bi:
    case CODE_FOR_mxu3_sr16macuub:
    case CODE_FOR_mxu3_sr16macsub:
    case CODE_FOR_mxu3_sr16macssb:
    case CODE_FOR_mxu3_sr16macssh:
    case CODE_FOR_mxu3_s16macuub:
    case CODE_FOR_mxu3_s16macsub:
    case CODE_FOR_mxu3_s16macssb:
    case CODE_FOR_mxu3_s16macssh:
    case CODE_FOR_mxu3_sr16sum2bi:
    case CODE_FOR_mxu3_sr16sum4bi:
    case CODE_FOR_mxu3_sr16sumub:
    case CODE_FOR_mxu3_sr16sumuh:
    case CODE_FOR_mxu3_sr16sumsb:
    case CODE_FOR_mxu3_sr16sumsh:
    case CODE_FOR_mxu3_sr16sumw:
    case CODE_FOR_mxu3_repiw:
      elemsize = elemsize >> 1;

    case CODE_FOR_mxu3_repih:
      elemsize = elemsize >> 1;

    case CODE_FOR_mxu3_repib:
    case CODE_FOR_mxu3_bshli:
    case CODE_FOR_mxu3_bshri:
      ...
      if (nops == 6) /* nargs = 5. SxMACxx */
      { 
        rangehi = (16 / elemsize) - 1;
	rangelo = 0;
	if (!IN_RANGE (INTVAL (ops[5].value), rangelo, rangehi))
        {
	  error_opno = 5;
          break;
        }
    ...
    case CODE_FOR_mxu3_movo:
      elemsize = elemsize >> 1;

    case CODE_FOR_mxu3_movq:
      elemsize = elemsize >> 1;

    case CODE_FOR_mxu3_movd:
      elemsize = elemsize >> 1;

    case CODE_FOR_mxu3_movw:
      elemsize = elemsize >> 2;

      if (CONST_INT_P (ops[2].value) && CONST_INT_P (ops[4].value))
      {
	  rangelo = 0;
	  rangehi = elemsize - 1;
	  ...
	  if (!IN_RANGE (INTVAL (ops[2].value), rangelo, rangehi))
	    ...
	    if (!IN_RANGE (INTVAL (ops[4].value), rangelo, rangehi))
	      error_opno = 4;
      }
      break;
    ...
    case CODE_FOR_mxu3_gt8w:
      esize = esize << 1;

    case CODE_FOR_mxu3_gt4w:
    case CODE_FOR_mxu3_gt4d:
      esize = esize << 1;

    case CODE_FOR_mxu3_gt2w:
    case CODE_FOR_mxu3_gt2d:
      esize = esize << 1;
    ...
    case CODE_FOR_mxu3_srarib:
    case CODE_FOR_mxu3_srarih:
    case CODE_FOR_mxu3_srariw:
    case CODE_FOR_mxu3_srlrib:
    case CODE_FOR_mxu3_srlrih:
    case CODE_FOR_mxu3_srlriw:
      gcc_assert (has_target_p && nops == 3);
      if (CONST_INT_P (ops[2].value))
	{
	  rangelo = 0;
    ...
    case CODE_FOR_mxu3_sllib:
    case CODE_FOR_mxu3_sllih:
    case CODE_FOR_mxu3_slliw:
    case CODE_FOR_mxu3_sraib:
    case CODE_FOR_mxu3_sraih:
    case CODE_FOR_mxu3_sraiw:
    case CODE_FOR_mxu3_srlib:
    case CODE_FOR_mxu3_srlih:
    case CODE_FOR_mxu3_srliw:
    ...
    case CODE_FOR_mxu3_extuwll:
    case CODE_FOR_mxu3_extuwlh:
    case CODE_FOR_mxu3_extuwhl:
    case CODE_FOR_mxu3_extuwhh:
    case CODE_FOR_mxu3_extudll:
    case CODE_FOR_mxu3_extudlh:
    case CODE_FOR_mxu3_extudhl:
    case CODE_FOR_mxu3_extudhh:
    case CODE_FOR_mxu3_extuqll:
    case CODE_FOR_mxu3_extuqlh:
    case CODE_FOR_mxu3_extuqhl:
    case CODE_FOR_mxu3_extuqhh:
    case CODE_FOR_mxu3_extuoll:
    case CODE_FOR_mxu3_extuolh:
    case CODE_FOR_mxu3_extuohl:
    case CODE_FOR_mxu3_extuohh:
      esize = esize << 1;
	  ...
	  if (!IN_RANGE (INTVAL (ops[3].value), rangelo, rangehi))
	    error_opno = 3;
      }
      break;
    ...
    case CODE_FOR_mxu3_law:
    case CODE_FOR_mxu3_lad:
    case CODE_FOR_mxu3_laq:
    case CODE_FOR_mxu3_lao:
      gcc_assert (nops == 5);
      rangelo = 0;
      rangehi = (64 / GET_MODE_UNIT_SIZE (ops[1].mode)) - 1;
      if (!CONST_INT_P (ops[2].value) || !IN_RANGE (INTVAL (ops[2].value), rangelo, rangehi))
        {
          error_opno = 2;
          break;
        }
      ...
      rangelo = -16;
      rangehi = 15;
      if (!CONST_INT_P (ops[4].value) || !IN_RANGE (INTVAL (ops[4].value), rangelo, rangehi))
        error_opno = 4;
      break;

    case CODE_FOR_mxu3_saw:
    case CODE_FOR_mxu3_sad:
    case CODE_FOR_mxu3_saq:
    case CODE_FOR_mxu3_sao:
      gcc_assert (nops == 4);
      rangelo = 0;
      rangehi = (64 / GET_MODE_UNIT_SIZE (ops[0].mode)) - 1;
      if (!CONST_INT_P (ops[1].value) || !IN_RANGE (INTVAL (ops[1].value), rangelo, rangehi))
        {
          error_opno = 1;
          break;
        }
      ...
      rangelo = -16;
      rangehi = 15;
      if (!CONST_INT_P (ops[3].value) || !IN_RANGE (INTVAL (ops[3].value), rangelo, rangehi))
        error_opno = 3;
      break;

    case CODE_FOR_mxu3_law2b:
    case CODE_FOR_mxu3_law2h:
    case CODE_FOR_mxu3_lad2b:
    case CODE_FOR_mxu3_lad2h:
    case CODE_FOR_mxu3_lad2w:
    case CODE_FOR_mxu3_laq2b:
    case CODE_FOR_mxu3_laq2h:
    case CODE_FOR_mxu3_laq2w:
    case CODE_FOR_mxu3_laq2d:
    case CODE_FOR_mxu3_lao2b:
    case CODE_FOR_mxu3_lao2h:
    case CODE_FOR_mxu3_lao2w:
    case CODE_FOR_mxu3_lao2d:
    case CODE_FOR_mxu3_lao2q:
      elemsize = elemsize >> 1;
    case CODE_FOR_mxu3_law4b:
    case CODE_FOR_mxu3_law4h:
    case CODE_FOR_mxu3_lad4b:
    case CODE_FOR_mxu3_lad4h:
    case CODE_FOR_mxu3_lad4w:
    case CODE_FOR_mxu3_laq4b:
    case CODE_FOR_mxu3_laq4h:
    case CODE_FOR_mxu3_laq4w:
    case CODE_FOR_mxu3_laq4d:
      elemsize = elemsize >> 1;
    case CODE_FOR_mxu3_law8b:
    case CODE_FOR_mxu3_law8h:
    case CODE_FOR_mxu3_lad8b:
    case CODE_FOR_mxu3_lad8h:
    case CODE_FOR_mxu3_lad8w:
      elemsize = elemsize >> 1;
    case CODE_FOR_mxu3_law16b:
    case CODE_FOR_mxu3_law16h:
      elemsize = elemsize >> 2;

      gcc_assert (nops == 5);
      rangelo = 0;
      rangehi = (64 / (GET_MODE_UNIT_SIZE (ops[1].mode) * elemsize)) - 1; 
      gcc_assert (rangehi >= 0);
      if (!CONST_INT_P (ops[2].value) || !IN_RANGE (INTVAL (ops[2].value), rangelo, rangehi))
        {
          error_opno = 2;
          break;
    ...
    case CODE_FOR_mxu3_sad2w:
    case CODE_FOR_mxu3_saq2w:
    case CODE_FOR_mxu3_saq2d:
    case CODE_FOR_mxu3_sao2w:
    case CODE_FOR_mxu3_sao2d:
    case CODE_FOR_mxu3_sao2q:
      elemsize = elemsize >> 1;
    case CODE_FOR_mxu3_sad4w:
    case CODE_FOR_mxu3_saq4w:
    case CODE_FOR_mxu3_saq4d:
      elemsize = elemsize >> 1;
    case CODE_FOR_mxu3_sad8w:
      elemsize = elemsize >> 3;

      gcc_assert (nops == 4);
      rangelo = 0;
      rangehi = (64 / (GET_MODE_UNIT_SIZE (ops[0].mode) * elemsize)) - 1;
      gcc_assert (rangehi >= 0);
      if (!CONST_INT_P (ops[1].value) || !IN_RANGE (INTVAL (ops[1].value), rangelo, rangehi))
        {
          error_opno = 1;
...
mips_reorg_process_insns (void)
{
  ...
  cfun->machine->machine_reorg2_p = true;
...
mips_option_override (void)
{
  ...
  /* Prohibit Paired-Single and MSA and MXU2 combination.  This is software
  ...
  if (ISA_HAS_MXU2 && TARGET_PAIRED_SINGLE_FLOAT)
    error ("unsupported combination: %s", "-mmxu2 -mpaired-single");

  if (ISA_HAS_MXU3_1 && TARGET_PAIRED_SINGLE_FLOAT)
    error ("unsupported combination: %s", "-mmxu3.1 -mpaired-single");
  else if (ISA_HAS_MXU3 && TARGET_PAIRED_SINGLE_FLOAT)
    error ("unsupported combination: %s", "-mmxu3 -mpaired-single");

  if (TARGET_MXU2)
    {
      if (TARGET_MSA)
	error ("unsupported combination: %s", "-mmxu2 -mmsa");
      else if (TARGET_FLOAT64)
	error ("unsupported combination: %s", "-mmxu2 -mfp64");
    }

  if (TARGET_MXU3_1)
    {
      if (TARGET_MXU2)
	error ("unsupported combination: %s", "-mmxu3.1 -mmxu2");
      else if (TARGET_MSA)
	error ("unsupported combination: %s", "-mmxu3.1 -mmsa");
    }
  else if (TARGET_MXU3)
    {
      /* disable the schedule optimization for MXU3.  */
      //global_options.x_flag_schedule_insns_after_reload = 0;
      //global_options.x_flag_schedule_insns = 0;
      
      if (TARGET_MXU2)
	error ("unsupported combination: %s", "-mmxu3 -mmxu2");
      else if (TARGET_MSA)
	error ("unsupported combination: %s", "-mmxu3 -mmsa");
    }

  if (TARGET_MXU3_EXT_ABI && !(TARGET_MXU3 || TARGET_MXU3_1))
    error ("%<-mmxu3-ext-abi%> requires %<-mmxu3%>/%<-mmxu3.1%>");
  ...
  if (!TARGET_OLDABI || TARGET_MXU3_EXT_ABI)
...
mips_conditional_register_usage (void)
{
  ...
  /* Add some registers in VPR and VWR register classes
     into the Callee-Saved set when enable MXU3 extended SIMD ABI. */
  if (TARGET_MXU3 && TARGET_MXU3_EXT_ABI)
    {
      int regno;
  ...
  call_really_used_regs[regno] = call_used_regs[regno] = 0;
      ...
      for (regno = VWR_REG_FIRST + 10; regno <= VWR_REG_LAST; regno++)
  call_really_used_regs[regno] = call_used_regs[regno] = 0;
    }
}
...
mips_expand_vector_init (rtx target, rtx vals)
{
  ...
  if (ISA_HAS_MXU3)
    {
	/* ERROR: #when same is INTVAL ,GET_MODE(same) is VOID #
      if (imode != GET_MODE(same))
        {
          error ("MXU3 vector initialization mode are not equal %s %s",
                 GET_MODE_NAME (imode), GET_MODE_NAME (GET_MODE (same)));
          gcc_unreachable ();
        }
    */
      if (all_same)
        {
          ...
          gcc_assert (!CONST_INT_P (same) && all_same);
	  temp = copy_to_mode_reg (imode, same);
          ...
          switch (vmode)
            ...
            default:
              error ("MXU3 vector initialization can not support %s",
                     GET_MODE_NAME (vmode));
              gcc_unreachable ();
            }
        }
        ...
        {
          #define MAX_MXU3_VECT_LEN 64
          int init_reg = 0;
          typedef struct mxu3_elem_stat {
            int cnt;
            int reg;
            int pos;
          } mxu3_elem_stat;
          mxu3_elem_stat elem_stat[MAX_MXU3_VECT_LEN];
          memset(elem_stat, -1, MAX_MXU3_VECT_LEN * sizeof(mxu3_elem_stat));
	  for (i = 0; i < nelt; i++)
            {
              int regno = REGNO (XVECEXP (vals, 0, i));
              for (int j = 0; j < MAX_MXU3_VECT_LEN; j++)
                {
                  if (elem_stat[j].reg == -1)
                    {
                      elem_stat[j].reg = regno;
                      elem_stat[j].cnt = 1;
                      elem_stat[j].pos = i;
                      break;
                    }
                  else if (elem_stat[j].reg == regno)
                    {
                      elem_stat[j].cnt++;
                      break;
                    }
	  ...
	  for (i = 1; i < MAX_MXU3_VECT_LEN; i++)
            {
              if (elem_stat[i].reg == -1)
                break;
              else if (elem_stat[i].cnt > elem_stat[i - 1].cnt)
                init_reg = i;
            }

          if (elem_stat[init_reg].cnt > 2)
            {
	      rtx temp = copy_to_mode_reg (imode, XVECEXP (vals, 0, elem_stat[init_reg].pos));
              mips_emit_move (target, gen_rtx_VEC_DUPLICATE (vmode, temp));
            }
          else
            {
	      ...
	      mips_emit_move (target, gen_rtx_CONST_VECTOR (vmode, vec));
            }
          #undef MAX_MXU3_VECT_LEN

	  for (i = 0; i < nelt; ++i)
	    {
              //if (REGNO (XVECEXP (vals, 0, i)) == elem_stat[init_reg].reg)
              //  continue;
		...
		{
		case V16SFmode:
		  emit_insn (gen_vec_setv16sf (target, temp, GEN_INT (i)));
		  break;

		case V8DImode:
		  emit_insn (gen_vec_setv8di (target, temp, GEN_INT (i)));
		  break;

		case V16SImode:
		  emit_insn (gen_vec_setv16si (target, temp, GEN_INT (i)));
		  break;

		case V32HImode:
		  emit_insn (gen_vec_setv32hi (target, temp, GEN_INT (i)));
		  break;

		case V64QImode:
		  emit_insn (gen_vec_setv64qi (target, temp, GEN_INT (i)));
		  break;
  ...
  if (ISA_HAS_MSA)
      ...
      if (all_same)
	{
	  ...
	  temp = copy_to_mode_reg (imode, temp2);
  ...
  /* vector init for MXU2 */
  if (ISA_HAS_MXU2 && MXU2_SUPPORTED_MODE_P(vmode))
    {
      if (all_same)
	{
	  rtx same = XVECEXP (vals, 0, 0);
	  ...
	  if (CONST_INT_P (same) && nvar == 0
	      && mips_signed_immediate_p (INTVAL (same), 15, 0))
	    {
	      switch (vmode)
		{
		case V16QImode:
	  ...
	  switch (vmode)
	    ...
	    case V16QImode:
	      ...
	      emit_insn (gen_mxu2_mfcpu_b (target, temp2));
	      break;

	    case V8HImode:
	      temp2 = simplify_gen_subreg (SImode, temp, imode, 0);
	      emit_insn (gen_mxu2_mfcpu_h (target, temp2));
	      break;

	    case V4SImode:
	      emit_insn (gen_mxu2_mfcpu_w (target, temp));
	      break;

	    case V2DImode:
	      emit_insn (gen_mxu2_mfcpu_d (target, temp));
	      break;

	    case V4SFmode:
	      emit_insn (gen_mxu2_mffpu_w_f (target, temp));
	      break;

	    case V2DFmode:
	      emit_insn (gen_mxu2_mffpu_d_f (target, temp));
	      break;
...
#if 0
static void
mips_expand_mxu2_cmp (rtx dest, enum rtx_code cond, rtx op0, rtx op1)
{
  machine_mode cmp_mode = GET_MODE (op0);
  bool negate = false;
	  ...
	  if (V4SFmode)
	    {
	      ...
	      emit_insn(gen_mxu2_fcor_w (temp1, op0, op1));
	      emit_insn(gen_mxu2_norv_w (temp1, temp1, temp1));
	      emit_insn(gen_mxu2_fceq_w (temp2, op0, op1));
	      emit_insn(gen_iorv4si3 (dest, temp1, temp2));
	    }
	  else
	    {
	      rtx temp1 = gen_reg_rtx (V2DImode);
	      rtx temp2 = gen_reg_rtx (V2DImode);
	      emit_insn(gen_mxu2_fcor_d (temp1, op0, op1));
	      emit_insn(gen_mxu2_norv_d (temp1, temp1, temp1));
	      emit_insn(gen_mxu2_fceq_d (temp2, op0, op1));
	      emit_insn(gen_iorv2di3 (dest, temp1, temp2));
	    }
	  break;
	default:
	  gcc_unreachable ();
...
mips_expand_mxu2_cmp (rtx dest, enum rtx_code cond, rtx op0, rtx op1)
{
  ...
  if (cmp_mode == V16QImode || cmp_mode == V8HImode 
      || cmp_mode == V4SImode || cmp_mode == V2DImode)
  {
      switch (cond)
	{
	case EQ:
  ...
  else if (cmp_mode == V4SFmode || cmp_mode == V2DFmode)
  {
      switch (cond)
	{
	case ORDERED:
			   ...
			   rtx *operands)
{
  ...
  if (ISA_HAS_MXU2)
    mips_expand_mxu2_cmp (cmp_res, GET_CODE (cond), cmp_op0, cmp_op1);
  else
...
/* Implement TARGET_SCHED_MACRO_FUSION_P.  */

static bool
mips_macro_fusion_p(void)
{
  return TARGET_MACRO_FUSION || TUNE_XBURST2;
}

static bool
mips_can_fuse_lwc1_pair_p (rtx_insn* prev, rtx_insn* curr)
{
  if (!TARGET_HARD_FLOAT_ABI || !TUNE_XBURST2
      || reload_completed)
    return false;

  /* Normally, the prev set insn should be lowered as a single 'lwc1'
     rather than a 'lw' and a following 'mtc1'. But due to the insn
     scheduling strategy for some multi-issued CPUs, e.g. XBURST2,
     these two insns may be apart before ira pass which breaks some
     validations for this optimization. To avoid this, we should pack
     them into a sched group here.
     */
  rtx prev_set = single_set (prev);
  rtx curr_set = single_set (curr);

  if (!prev_set || !curr_set 
      || !MEM_P (SET_SRC (prev_set)) 
      || !REG_P (SET_DEST (prev_set))
      || GET_CODE (SET_SRC (curr_set)) != FLOAT
      || GET_MODE (SET_DEST (curr_set)) != DFmode)
    return false;

  rtx reg = XEXP (SET_SRC (curr_set), 0);
  /* This pair can be fused if the dest of prev insn is
     only used by curr insn. */
  if (REG_P (reg) && REGNO (reg) == REGNO (SET_DEST (prev_set))
      && find_reg_note (curr, REG_DEAD, reg))
    return true;
...
static bool
mips_can_fusiable_pair_check(rtx_insn* prev, rtx_insn* curr)
{
  rtx prev_set = single_set (prev);
  rtx curr_set = single_set (curr);
  ...
  if (TARGET_64BIT)
    return false;
  if (!TARGET_MACRO_FUSION)
    return false;
  if (!prev_set || !curr_set)
    return false;
  if (any_uncondjump_p (curr) || any_uncondjump_p (prev))
    return false;
  if (any_condjump_p (curr) || any_condjump_p (prev))
    return false;

  /* Just fusion ALU MOVE MOVE_COST insn. */
  if ((get_attr_alu_type(prev) == ALU_TYPE_UNKNOWN
       && get_attr_move_type(prev) != MOVE_TYPE_MOVE
       && get_attr_move_type(prev) != MOVE_TYPE_LOAD
       && get_attr_move_type(prev) != MOVE_TYPE_CONST
       && get_attr_type(prev) != TYPE_SHIFT
       && get_attr_type(prev) != TYPE_SLT
       && get_attr_got(prev) != GOT_LOAD)
      ||(get_attr_alu_type(curr) == ALU_TYPE_UNKNOWN
	 && get_attr_move_type(curr) != MOVE_TYPE_MOVE
	 && get_attr_move_type(curr) != MOVE_TYPE_STORE
	 && get_attr_move_type(curr) != MOVE_TYPE_CONST
	 && get_attr_type(curr) != TYPE_SHIFT
	 && get_attr_type(curr) != TYPE_SLT))
    return false;

  /* Just fusion Integer instruction */
  if (GET_MODE (SET_DEST (curr_set)) != SImode)
    return false;

  if ((XINT (SET_SRC (prev_set), 1) != UNSPEC_LOAD_GOT
       && GET_RTX_CLASS (GET_CODE (SET_SRC (prev_set))) == RTX_EXTRA)
      || GET_RTX_CLASS (GET_CODE (SET_SRC (curr_set))) == RTX_EXTRA)
    return false;

  return true;
}

/* Implement TARGET_SCHED_MACRO_FUSION_PAIR_P. */

static bool
mips_macro_fusion_pair_p(rtx_insn* prev, rtx_insn* curr)
{
  rtx p_dest, c_dest, p_src, c_src;
  rtx prev_set = single_set (prev);
  rtx curr_set = single_set (curr);

  if (mips_can_fuse_lwc1_pair_p (prev, curr))
    return true;

  if (!mips_can_fusiable_pair_check(prev, curr))
    return false;

  p_dest = SET_DEST (prev_set);
  p_src = SET_SRC (prev_set);
  c_dest = SET_DEST (curr_set);
  c_src = SET_SRC (curr_set);

  if (REG_P (p_dest) && get_attr_move_type(curr) == MOVE_TYPE_STORE)
    {
      if (!REG_P (c_src))
	return false;

      if ((reload_completed
	   || (HARD_REGISTER_NUM_P (REGNO (p_dest))
	       && HARD_REGISTER_NUM_P(REGNO (c_src)))) ?
	  (REGNO (p_dest) == REGNO (c_src)) : 1)
	{
	  if (GET_RTX_CLASS(GET_CODE (p_src)) == RTX_EXTRA
	      || GET_RTX_CLASS(GET_CODE (p_src)) == RTX_CONST_OBJ
	      || GET_RTX_CLASS(GET_CODE (p_src)) == RTX_OBJ
	      || GET_RTX_CLASS(GET_CODE (p_src)) == RTX_UNARY
	      || CONST_INT_P (XEXP (p_src, 1)))
	    return true;
	}
      return false;
    }

  if (REG_P (p_dest) && REG_P (c_dest))
      {
	unsigned int check_reg;
	unsigned int p_regno = REGNO (p_dest);
	unsigned int c_regno = REGNO (c_dest);
	enum rtx_class c_class = GET_RTX_CLASS(GET_CODE (c_src));
	enum rtx_class p_class = GET_RTX_CLASS(GET_CODE (p_src));

	if ((reload_completed
	     || (HARD_REGISTER_NUM_P(p_regno) && HARD_REGISTER_NUM_P(c_regno))) ?
	    (p_regno == c_regno) : 1)
	  {
	    /* Trying to ignore
	       curr(move) = (set (reg) (const_int imm16))
	       or curr(move) = (set (reg) (reg)). */
	    if (GET_CODE (c_src) != LO_SUM
		&& (c_class == RTX_CONST_OBJ || c_class == RTX_OBJ))
	      return false;

	    if (GET_CODE (c_src) == LO_SUM
		|| c_class == RTX_UNARY || CONST_INT_P (XEXP (c_src, 1)))
	      {
		/* Trying to match:
		   prev = (set (reg) (*))
		   curr = (set (reg) (nor (reg)));
		   or curr = (set (reg) (and (reg) (const_int imm16)));
		   or curr = (set (reg) (lo_sum: ((reg) (*)))).

		   Need (prd == crd && prd == crs).
		   Is XEXP (c_src, 0) ok for *muls? */
		if (REG_P (XEXP (c_src, 0)) &&
		    REGNO (p_dest) == REGNO (XEXP (c_src, 0)))
		  return true;
	      }
	    else
	      {
		/* Trying to match curr = (set (reg) (plus (reg) (reg)))*/
		if (REG_P (XEXP (c_src, 0)) && REG_P (XEXP (c_src, 1)))
		  {
		    if ((REGNO (XEXP (c_src, 0)) == p_regno))
		      check_reg = REGNO (XEXP (c_src, 1));
		    else if (REGNO (XEXP (c_src, 1)) == p_regno)
		      check_reg = REGNO (XEXP (c_src, 0));
		    else
		      return false;
		    /* prev = (set (reg) (nor (reg)))
		       or prev = (set (reg) (lo_sum (*))).
		       Need (prd == crd && (prd == crs || prd == crt). */
		    if (p_class == RTX_UNARY
			|| p_class == RTX_CONST_OBJ || p_class == RTX_OBJ
			|| p_class == RTX_EXTRA)
		      return true;
		    /* prev = (set (reg) (and (reg) (const_int imm16))); */
		    else if (CONST_INT_P (XEXP (p_src, 1)))
		      return true;
		    /* prev = (set (reg) (and (reg) (reg))); */
		    else if (REG_P (XEXP (p_src, 0)) && REG_P (XEXP (p_src, 1)))
		      if (check_reg == REGNO (XEXP (p_src, 1)) ||
			  check_reg == REGNO (XEXP (p_src, 0)))
			return true;
		  }
		return false;
	      }
...
/* Implement the TARGET_SCHED_FUSION_PRIORITY hook.
   ...
   Currently we only support to fuse MXU3 load or MXU3 store instructions, so FUSION_PRI
...
{
  rtx x, dest, src, op0, op1, op2;
  RTX_CODE dest_code, src_code, op0_code, op1_code, op2_code;
  machine_mode dest_mode, src_mode, op0_mode, op1_mode, op2_mode;
  int curr_fusion_pri = max_pri - 1, curr_pri = max_pri - 1, base_fusion_pri = 100;
  unsigned int curr_base;
  enum mips_fusion_type curr_type = INSN_NULL;
  ...
  if (TARGET_MXU3)
    {
      ...
      if (GET_CODE (x) == SET)
        {
          ...
          if ((dest_code == MEM) && (src_code == VEC_SELECT))
            {
              gcc_assert(dest_mode == src_mode);
              op0 = XEXP (dest, 0);
              op0_code = GET_CODE(op0);
              if (op0_code == PLUS) 
                {
                  dest = op0;
                  op0 = XEXP (dest, 0);
                  ...
                  op1 = XEXP (dest, 1);
                  op1_code = GET_CODE(op1);
                  if ((op0_code == REG) && (op1_code = CONST_INT))
                    {
                      curr_base = REGNO (op0);
                      curr_type = INSN_MXU3_STORE; 
                    }
                }
            }
          else if ((dest_code == REG) && (src_code  == VEC_MERGE))
            {
              op0 = XEXP (src, 0);
              op0_code = GET_CODE (op0);
              op0_mode = GET_MODE (op0);     
              ...
              op2 = XEXP (src, 2);
              op2_code = GET_CODE (op2);
              op2_mode = GET_MODE (op2);     

              if ((op0_code == VEC_DUPLICATE) && (op1_code == REG) && (op2_code == CONST_INT))
                {
                  src = op0;
                  op0 = XEXP (src, 0);
                  op0_code = GET_CODE (op0);
                      ...
                      if (op0_code == PLUS)
                        {
                          ...
                          if ((op0_code == REG) && (op1_code == CONST_INT))
                            {
                              curr_base = REGNO (op0);
                              curr_type = INSN_MXU3_LOAD; 
                            }
                        }
                    }
                } 
            }
          else if ((dest_code ==  REG) && (dest_mode == SImode) &&
                   (src_code  == PLUS) && (src_mode  == SImode))
            {
              op0 = XEXP (src, 0);
              op0_code = GET_CODE (op0);
              op0_mode = GET_MODE (op0);     
              ...
              if ((op0_code == REG) && (op0_mode == SImode) &&
                  (op1_code == CONST_INT))
                {
                  curr_base = REGNO (op0);
                  curr_type = INSN_PLUS; 
      ...
      if (curr_type != INSN_NULL)
        {
          curr_fusion_pri = base_fusion_pri + curr_base;
          curr_pri = curr_fusion_pri;
        }
    }

  *fusion_pri = curr_fusion_pri;
  *pri = curr_pri;
  return;
}

/* Check MXU3 load/store peephole2 condtion and preparation operands. */
bool
mips_mxu3_ls_peephole2_p (int * key, rtx ** op0, rtx ** op1, rtx ** op2,
                          rtx * inc, machine_mode mode, int len)
{
  int width = GET_MODE_SIZE (mode);
  int * sort_key = (int *)xmalloc(sizeof(int) * len);
  ...
  std::sort(sort_key, sort_key + len);
  for (int i = 0; i < len; i++)
    {
      if (sort_key[i] != width * i)
        {
          free(sort_key);
          return false;
        }
          ...
          if (key[j] > key[j + 1])
            {
              ...
              if (op0 != NULL)
                std::swap(*(op0[j]), *(op0[j + 1]));
              if (op1 != NULL)
                std::swap(*(op1[j]), *(op1[j + 1]));
              if (op2 != NULL)
                std::swap(*(op2[j]), *(op2[j + 1]));
            }
        }
    }
  *inc = GEN_INT (width);
				      ...
				      reg_class_t best_class ATTRIBUTE_UNUSED)
{
  ...
  /* Force assign V1SImode allocnos to VWR_REGS rather than MEM or some
     ineffient code will be inserted during the reload stage.
     Note we should manully handle some cases when moving between SImode
     and V1SImode, see `mips_legitimize_move` for details.*/
  if (ISA_HAS_MXU3 && PSEUDO_REGNO_MODE (regno) == V1SImode)
    return VWR_REGS;
...
static bool
mips_mxu3_invalid_compute_mode_p (machine_mode mode)
{
  gcc_assert (mode != TImode && mode != OImode
              && mode != XImode);

  return VECTOR_MODE_P (mode) && (mode == V1SImode
         || GET_MODE_UNIT_BITSIZE (mode) >= GET_MODE_BITSIZE (TImode));
}

/* Return the diagnostic message string if the unary operation OP is
   not permitted on TYPE, NULL otherwise. */
static const char *
mips_invalid_unary_op (int op, const_tree type)
{
  if (TARGET_MXU3
      && op != CONVERT_EXPR && op != ADDR_EXPR
      && mips_mxu3_invalid_compute_mode_p (TYPE_MODE (type)))
    return N_("invalid operation on unsupported vector type");
...
static const char *
mips_invalid_binary_op (int op, const_tree type1, const_tree type2)
{
  if (TARGET_MXU3)
    {
      ...
      /* Handle the special case for V1SImode, only binary add OP is
         supported by the MXU3 ISA. */
      if (op == PLUS_EXPR
          && mode1 == mode2 && mode1 == V1SImode)
        return NULL;

      if (mips_mxu3_invalid_compute_mode_p (mode1)
          || mips_mxu3_invalid_compute_mode_p (mode2))
        return N_("invalid operation on unsupported vector types");
    }
  return NULL;
}
...
#define TARGET_SCHED_MACRO_FUSION_P mips_macro_fusion_p
...
#define TARGET_INVALID_UNARY_OP mips_invalid_unary_op
#undef TARGET_INVALID_BINARY_OP
#define TARGET_INVALID_BINARY_OP mips_invalid_binary_op
