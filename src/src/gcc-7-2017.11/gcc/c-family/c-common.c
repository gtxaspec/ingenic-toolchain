...
vector_types_convertible_p (const_tree t1, const_tree t2, bool emit_lax_note)
{
  ...
  convertible_lax =
    ...
    //ylli modify 20210909 to modify option:'-flax-vector-conversions' 
    //Can allow implicit conversions between vectors with differing numbers of 
    //subparts and/or differing element type not only integral type
    /*(tree_int_cst_equal (TYPE_SIZE (t1), TYPE_SIZE (t2))
     && (TREE_CODE (TREE_TYPE (t1)) != REAL_TYPE ||
	 TYPE_VECTOR_SUBPARTS (t1) == TYPE_VECTOR_SUBPARTS (t2))
