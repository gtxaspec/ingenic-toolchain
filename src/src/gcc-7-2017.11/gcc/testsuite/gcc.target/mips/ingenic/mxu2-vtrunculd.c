...
int main (void)
{
  ...
  u1.x = __builtin_mxu2_vtrunculd(s1.x);
  v1.x = (v2u64){(1), (2),};

  if(check_ud (&u1, &v1, len))
    abort ();

  u2.x = __builtin_mxu2_vtrunculd(s2.x);
  v2.x = (v2u64){(3), (4),};
  if(check_ud (&u2, &v2, len))
    abort ();

  u3.x = __builtin_mxu2_vtrunculd(s3.x);
  v3.x = (v2u64){(5), (6),};
  if(check_ud (&u3, &v3, len))
    abort ();

  u4.x = __builtin_mxu2_vtrunculd(s4.x);
  v4.x = (v2u64){(7), (8),};
  if(check_ud (&u4, &v4, len))
    abort ();

  u5.x = __builtin_mxu2_vtrunculd(s5.x);
  v5.x = (v2u64){(0), (0),};
  if(check_ud (&u5, &v5, len))
    abort ();

  u6.x = __builtin_mxu2_vtrunculd(s6.x);
  v6.x = (v2u64){(0), (0),};
  if(check_ud (&u6, &v6, len))
    abort ();

  u7.x = __builtin_mxu2_vtrunculd(s7.x);
  v7.x = (v2u64){(0), (0),};
  if(check_ud (&u7, &v7, len))
    abort ();

  u8.x = __builtin_mxu2_vtrunculd(s8.x);
  v8.x = (v2u64){(0),(0),};
