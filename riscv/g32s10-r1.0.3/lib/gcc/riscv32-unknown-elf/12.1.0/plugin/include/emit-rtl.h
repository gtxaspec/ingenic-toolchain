...
struct GTY(()) rtl_data {
  ...
  /* Locate the vla slot.  */
  poly_int64_pod x_vla_frame_offset;
  ...
  /* Nonzero if current function must be given a vla local pointer.
     Set in lra-eliminations.cc if vla slot is allocated
     on the stack there.  */
  bool vla_local_pointer_needed;
...
#define vla_frame_offset (crtl->x_vla_frame_offset)
...
#define vla_local_pointer_needed (crtl->vla_local_pointer_needed)
