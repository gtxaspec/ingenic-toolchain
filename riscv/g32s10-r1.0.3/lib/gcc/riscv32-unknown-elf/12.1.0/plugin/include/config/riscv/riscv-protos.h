...
extern rtx riscv_gen_const_int_vector (machine_mode, HOST_WIDE_INT);
extern int riscv_const_vec_li_min_duplicate_bytes (rtx);
extern bool riscv_const_poly_int_p (rtx);
extern bool riscv_aie_vector_expand_const_vector (rtx, rtx);
extern int riscv_aie_address_offser_p (rtx, machine_mode);
extern rtx riscv_ingenicaie_add_offset (machine_mode, rtx, HOST_WIDE_INT);
extern rtx riscv_ingenicaie_add_poly_offset (rtx, rtx);
extern bool riscv_aie_vector_segment_dynamic_range_p (int, int, int);
extern const char* riscv_aie_vector_output_move (rtx *);
...
#define AIE_VUNDEF(MODE)                                                       \
  gen_rtx_UNSPEC (MODE, gen_rtvec (1, gen_rtx_REG (SImode, X0_REGNUM)),        \
		  UNSPEC_VUNDEF)

extern const riscv_cpu_info *riscv_find_cpu (const char *);
extern bool  riscv_1bit_select_p (rtx, machine_mode);
